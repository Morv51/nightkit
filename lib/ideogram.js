...
// Ideogram 4.0: Es gibt KEIN edit-v4 — das V4-Pendant zum bestehenden
// /v1/edit-Flow ist Remix (Template als Basisbild + Prompt). Laut Doku heißt
// das Prompt-Feld hier text_prompt (nicht prompt), und remix-v4 kennt kein
// magic_prompt-Feld — "magic_prompt OFF" ist damit implizit erfüllt.
// image_weight steuert, wie stark das Template das Ergebnis bindet (hoch =
// layouttreu). resolution 1440x2560 ist exakt 9:16 und spiegelt das fixe
// aspect_ratio 9x16 des V3-Pfads.
// Kosten: V4-Calls können anders bepreist sein als V3 — Preise in der
// Ideogram-Doku prüfen.
function remixV4({
  apiKey,
  textPrompt,
  imageBuffer,
  imageType = "image/png",
  resolution = "1440x2560",   // exakt 9:16, analog zum V3-aspect_ratio 9x16
  imageWeight = 90,           // justierbar: hoch = nah am Template-Layout
  renderingSpeed = "QUALITY", // beste verfügbare V4-Stufe
}) {
  if (!imageBuffer || !imageBuffer.length) return Promise.reject(new Error("Template image missing"));
  ...
  return postMultipart({
    ...
    files: [{ field: "image", filename: "template.png", contentType: imageType, buffer: imageBuffer }],
  });
}
...
//
// Laut Doku akzeptiert Reframe KEIN prompt / negative_prompt / magic_prompt
// (Request-Felder: image, resolution, color_palette, num_images,
// rendering_speed, seed, style_codes, style_preset, style_reference_images).
// Der Stil der erweiterten Flächen wird deshalb über style_reference_images
// gesteuert — zusammen mit einem festen seed der entscheidende Hebel gegen
// Fantasietext und falsche Hintergrundfarben in den neuen Flächen.
function reframe({
  apiKey,
  imageBuffer,
  imageType = "image/png",
  resolution,
  styleReferenceBuffer = null,
  styleReferenceType = "image/png",
  renderingSpeed = null, // FLASH | TURBO | DEFAULT | QUALITY
  seed = null,
}) {
  if (!imageBuffer || !imageBuffer.length) return Promise.reject(new Error("Image missing"));
  ...
  if (renderingSpeed) fields.rendering_speed = renderingSpeed;
  if (Number.isInteger(seed)) fields.seed = String(seed);

  const files = [{ field: "image", filename: "image.png", contentType: imageType, buffer: imageBuffer }];
  if (styleReferenceBuffer && styleReferenceBuffer.length) {
    files.push({
      field: "style_reference_images",
      filename: "style.png",
      contentType: styleReferenceType,
      buffer: styleReferenceBuffer,
    });
  }

  return postMultipart({ apiKey, path: REFRAME_V3_PATH, fields, files });
}
...
module.exports = { edit, editMasked, remixV4, reframe, download };
