...
//   OPENAI_TEXT_IMAGE_DETAIL Default "high" (low | high | auto)
...
//
// Env (Variantencall — aus EINEM Produktionsprompt N Fassungen):
//   OPENAI_VARIANTS_MODEL       Default "gpt-5.6-sol"
//   OPENAI_VARIANTS_EFFORT      Default "medium"
//   OPENAI_VARIANTS_VERBOSITY   Default "high"
//   OPENAI_VARIANTS_MAX_OUTPUT  ohne Wert: 8000 + 6000 je Variante
//   OPENAI_VARIANTS_TIMEOUT_MS  Default 900000 (15 min)
//
...
// Bild-Detailgrad der HAUPTANALYSE. "high" laesst das Modell das Bild in voller
// Aufloesung auswerten statt in der groben Vorschau. Erlaubt sind low | high |
// auto, alles andere faellt auf "high" zurueck. Der Vorschlags-Call sendet
// detail bewusst NICHT und bleibt damit beim Standard — dort geht es nur um
// Genre und Vibe.
const IMAGE_DETAIL_ALLOWED = ["low", "high", "auto"];
function pickDetail(v) {
  if (IMAGE_DETAIL_ALLOWED.indexOf(v) !== -1) return v;
  if (v != null && v !== "") console.warn('[studio-openai-text] Unbekanntes Bild-Detail "' + v + '" — nutze "high"');
  return "high";
}
const IMAGE_DETAIL = pickDetail(process.env.OPENAI_TEXT_IMAGE_DETAIL);
...
// Variantencall: EIN Produktionsprompt rein, N vollstaendige Fassungen raus.
// Verbosity bewusst "high" — die Varianten sollen die LAENGE des Originals halten,
// und "medium" hat bei der Hauptanalyse nachweislich verdichtet.
const VARIANTS_MODEL = process.env.OPENAI_VARIANTS_MODEL || "gpt-5.6-sol";
const VARIANTS_EFFORT = envOr("OPENAI_VARIANTS_EFFORT", "medium");
const VARIANTS_VERBOSITY = pickVerbosity(envOr("OPENAI_VARIANTS_VERBOSITY", "high"), "high");
const VARIANTS_TIMEOUT_MS = Number(process.env.OPENAI_VARIANTS_TIMEOUT_MS) || 900 * 1000;

// DAS Limit, an dem dieses Feature steht oder faellt: bei der Responses API zaehlen die
// Reasoning-Tokens gegen max_output_tokens. N vollstaendige Prompts "of the same length"
// sind grob 1500 Ausgabe-Tokens JE Variante. Ein fester Wert wie die 16000 der
// Hauptanalyse reisst bei hohen N unweigerlich — und dann ist der GANZE Call verloren,
// obwohl bezahlt. Darum waechst das Limit mit der Anzahl mit.
const VARIANTS_MAX_BASE = 8000;
const VARIANTS_MAX_PER = 6000;
function variantsMaxOutput(anzahl) {
  const fest = Number(process.env.OPENAI_VARIANTS_MAX_OUTPUT);
  if (Number.isFinite(fest) && fest > 0) return Math.round(fest);
  return VARIANTS_MAX_BASE + VARIANTS_MAX_PER * Math.max(1, Number(anzahl) || 1);
}
...
function analyzeImage({ apiKey, prompt, instructions = "", imageBase64, imageType = "image/png", imageDetail = IMAGE_DETAIL, model = MODEL, effort = EFFORT, verbosity = VERBOSITY, maxOutput = MAX_OUTPUT, timeoutMs = TIMEOUT_MS }) {
  return new Promise((resolve, reject) => {
    ...
    // null oder undefined = Feld vergessen. Leerstring ist erlaubt und
    // gewollt: der Variantencall laeuft absichtlich ohne Bild.
    if (imageBase64 == null) return reject(new Error("Referenzbild fehlt"));

    let settled = false;
    let killer = null;
    const done = (fn) => { if (settled) return; settled = true; if (killer) clearTimeout(killer); fn(); };

    // detail nur mitsenden, wenn gesetzt — der Vorschlags-Call gibt bewusst ""
    // mit und bleibt damit beim Standard.
    const inhalt = [{ type: "input_text", text: prompt }];
    if (imageBase64) {
      const bild = { type: "input_image", image_url: "data:" + imageType + ";base64," + imageBase64 };
      if (imageDetail) bild.detail = imageDetail;
      inhalt.push(bild);
    }
    ...
    console.log("[studio-openai-text] -> POST https://" + HOST + PATH + " (image->text) | model=" + model + " effort=" + (effort || "-") + " verbosity=" + (verbosity || "-") + " detail=" + (imageDetail || "-") + " maxOut=" + maxOutput + " promptLen=" + prompt.length + " instrLen=" + (instructions ? instructions.length : 0) + (imageBase64 ? "" : " (ohne Bild)"));
...
module.exports = {
  analyzeImage,
  MODEL, EFFORT, VERBOSITY, IMAGE_DETAIL, MAX_OUTPUT, TIMEOUT_MS,
  CLASSIFY_MODEL, CLASSIFY_EFFORT, CLASSIFY_VERBOSITY, CLASSIFY_MAX_OUTPUT, CLASSIFY_TIMEOUT_MS,
  VARIANTS_MODEL, VARIANTS_EFFORT, VARIANTS_VERBOSITY, VARIANTS_TIMEOUT_MS, variantsMaxOutput,
};
