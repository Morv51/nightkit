...
function analyzeImage({ apiKey, prompt, imageBase64, imageType = "image/png", model = MODEL, effort = EFFORT, verbosity = VERBOSITY, maxOutput = MAX_OUTPUT, timeoutMs = TIMEOUT_MS }) {
  return new Promise((resolve, reject) => {
    ...
    // Leerer Wert -> Feld faellt WEG (siehe Env-Hinweis oben).
    if (effort) koerper.reasoning = { effort };
    if (verbosity) koerper.text = { verbosity };
    const payload = JSON.stringify(koerper);

    console.log("[studio-openai-text] -> POST https://" + HOST + PATH + " (image->text) | model=" + model + " effort=" + (effort || "-") + " verbosity=" + (verbosity || "-") + " maxOut=" + maxOutput + " promptLen=" + prompt.length);
