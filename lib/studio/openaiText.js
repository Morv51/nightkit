...
function analyzeImage({ apiKey, prompt, instructions = "", imageBase64, imageType = "image/png", imageDetail = IMAGE_DETAIL, model = MODEL, effort = EFFORT, verbosity = VERBOSITY, maxOutput = MAX_OUTPUT, timeoutMs = TIMEOUT_MS }) {
  return new Promise((resolve, reject) => {
    ...
    console.log("[studio-openai-text] -> POST https://" + HOST + PATH + " (image->text) | model=" + model + " effort=" + (effort || "-") + " verbosity=" + (verbosity || "-") + " detail=" + (imageDetail || "-") + " maxOut=" + maxOutput + " promptLen=" + prompt.length + " instrLen=" + (instructions ? instructions.length : 0));
