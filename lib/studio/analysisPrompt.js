...
const VARIANTS_PATH = path.join(__dirname, "..", "..", "prompts", "variants.txt");
...
function loadVariants() { return loadFile(VARIANTS_PATH, "prompts/variants.txt"); }
...
async function suggest({ apiKey, imageBase64, imageType }) {
  ...
  const res = await openaiText.analyzeImage({
    ...
    imageDetail: "",                       // Standard beibehalten: hier geht es nur um Genre und Vibe
...
// ── Varianten ───────────────────────────────────────────────────────────────
// EIN fertiger Produktionsprompt rein, N vollstaendige Fassungen raus. Reiner
// Textcall, kein Bild. prompts/variants.txt geht UNVERAENDERT als instructions;
// der Eingabetext traegt nur die Anzahl und den Prompt.

// Trennung an einer Zeile, die NUR --- enthaelt (so verlangt es die Vorlage).
// Leere Stuecke fliegen raus, der Rest wird randbereinigt.
...
async function makeVariants({ apiKey, prompt, anzahl }) {
  const n = Math.max(1, Math.min(10, parseInt(anzahl, 10) || 1));
  const instructions = loadVariants();
  const maxOutput = openaiText.variantsMaxOutput(n);
  const eingabe = "Produce exactly " + n + " variants.\n\n" + prompt;

  const res = await openaiText.analyzeImage({
    apiKey, prompt: eingabe, instructions,
    imageBase64: "",                        // reiner Textcall — kein Bild, kein detail
    model: openaiText.VARIANTS_MODEL,
    effort: openaiText.VARIANTS_EFFORT,
    verbosity: openaiText.VARIANTS_VERBOSITY,
    maxOutput,
    timeoutMs: openaiText.VARIANTS_TIMEOUT_MS,
  });

  const teile = splitVariants(res.text);
  // ZU WENIG ist ein harter Fehler: sonst liefe ein Lauf mit weniger Kacheln an, als
  // angefordert wurden, ohne dass es jemand merkt.
  if (teile.length < n) {
    const e = new Error(
      "Variantencall lieferte nur " + teile.length + " von " + n + " Varianten. "
      + "Moeglicherweise war das Ausgabelimit (" + maxOutput + " Tokens) zu klein — "
      + "dann hilft OPENAI_VARIANTS_MAX_OUTPUT."
    );
    e.tooFew = true;
    throw e;
  }
  // ZU VIEL ist harmlos: die ersten N nehmen, den Rest verwerfen und es melden.
  return { prompts: teile.slice(0, n), geliefert: teile.length, angefordert: n, maxOutput };
}

module.exports = { run, buildPrompt, loadMaster, MASTER_PATH, suggest, loadGenreVibe, parseGenreVibe, GENRE_VIBE_PATH, loadInstructions, INSTRUCTIONS_PATH, makeVariants, loadVariants, splitVariants, VARIANTS_PATH };
