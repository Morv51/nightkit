...
const openaiImage = require("./openaiImage");      // 2. Bild-Engine (GPT Image)
const segmind = require("./segmind");              // Auto-Flow-Engine: Nano Banana Pro
...
// safety_tolerance 1 (strikt) … 6 (am wenigsten streng). Default 6 (permissivste
// Stufe, damit Club-Motive nicht unnötig geblockt werden); per Env übersteuerbar.
const SEGMIND_SAFETY = (() => {
  const n = parseInt(process.env.SEGMIND_SAFETY_TOLERANCE, 10);
  return n >= 1 && n <= 6 ? n : 6;
})();
// Ideogram-V3-Erstellen (Auto-Flow): magic_prompt. Default OFF (erst ohne testen);
// per Env IDEOGRAM_STUDIO_MAGIC=ON auf AN zum Gegentest, ohne Code-Änderung.
const IDEOGRAM_STUDIO_MAGIC = process.env.IDEOGRAM_STUDIO_MAGIC === "ON" ? "ON" : "OFF";
...
// Temporärer Referenzbild-Speicher: Segmind (Nano Banana Pro) lädt image_urls als
// ÖFFENTLICHE URL (kein Base64). Wir legen das hochgeladene Bild kurz in den
// Speicher und liefern es unter /studio-ref/<zufalls-id> aus (öffentlich, damit
// Segmind es abholen kann; unrat-bare Zufalls-ID + 15-Min-TTL). Nach dem Job
// wird es entfernt; ein Sweeper räumt Reste weg.
const refImages = new Map(); // id → { buffer, contentType, exp }
function putRefImage(buffer, contentType) {
  const id = crypto.randomBytes(8).toString("hex");
  refImages.set(id, { buffer, contentType: contentType || "image/png", exp: Date.now() + 15 * 60 * 1000 });
  return id;
}
setInterval(() => {
  const now = Date.now();
  for (const [id, r] of refImages) if (r.exp < now) refImages.delete(id);
}, 60 * 1000).unref?.();
...
function register(router) {
  ...
  router.post("/admin/analyze", async (req, res) => {
    ...
    try {
      ...
      const prompt = promptBuilder.buildMoodboardPrompt(dna, { refType });
  ...
  router.post("/admin/build-prompt", async (req, res) => {
    ...
    const b = await body(req, res);
    if (!b) return true;
    const prompt = promptBuilder.buildMoodboardPrompt(b.dna || {}, {
  ...
  // ── Auto-Flow (Beta): Ein-Flyer-Generierung via NANO BANANA PRO über die
  //    Segmind-API. Referenzbild (image_urls, öffentliche Temp-URL) + erzeugter
  //    Prompt → neues Template. Aspect 9:16, safety_tolerance permissiv (6).
  //    KEIN Ideogram, KEINE ChatGPT-Weboberfläche.
  //
  //    ASYNCHRON (Job-Muster wie /api/generate): sofort 202 + jobId, Generierung
  //    läuft im HINTERGRUND, das Frontend pollt /admin/auto-generate/<jobId>. So
  //    bleibt KEIN Inbound-Request lange offen → kein Render-Gateway-502. Eine
  //    inhaltliche Ablehnung von Segmind kommt als Klartext-Fehler im Job zurück. ──
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    if (!b.prompt) return sendError(res, 400, "Prompt fehlt");
    const pic = parseDataUrl(b.image);
    if (!pic) return sendError(res, 400, "Referenzbild fehlt oder ist ungültig (PNG/JPEG/WebP, ≤10MB)");

    // ── Ideogram V3: ERSTELLEN mit Style-Reference (SEPARATER Aufruf, NICHT der
    //    produktive edit()-Befüll-Flow). Referenzbild geht als Style-Reference-
    //    Buffer direkt mit (keine öffentliche URL nötig) + unser Prompt → neuer
    //    Flyer. magic_prompt per Env schaltbar (Default OFF), 9x16, höchste
    //    Qualität. Ideogram hat keinen Moderations-Parameter (Ablehnung = Fehler). ──
    if (b.model === "ideogram-v3") {
      if (!IDEOGRAM_KEY) return sendError(res, 500, "Ideogram nicht konfiguriert — IDEOGRAM_API_KEY in Render setzen");
      // Ideogram rendert Text am zuverlässigsten, wenn die EXAKTEN Wörter klar als
      // zu setzender Text markiert sind. Unseren (unveränderten, generischen)
      // Prompt daher NUR für den Ideogram-Aufruf um eine Klartext-Liste der exakten
      // Platzhalter in Anführungszeichen ergänzen — buildMoodboardPrompt bleibt gleich.
      const exactTexts = Object.values(promptBuilder.ROLE_PLACEHOLDERS).map((t) => `"${t}"`).join(", ");
      const ideoPrompt = b.prompt +
        "\n\nIMPORTANT — the flyer text must be EXACTLY these placeholder words, each spelled exactly as " +
        "written and kept as one intact word/line, and NO other words, captions or titles may be invented: " +
        exactTexts + ".";
      ...
      (async () => {
        const { url } = await ideogramV3.generateWithStyleRef({
          apiKey: IDEOGRAM_KEY, prompt: ideoPrompt,
          styleRefBuffers: [pic.buffer], styleRefType: pic.mediaType,
          aspectRatio: "9x16", magicPrompt: IDEOGRAM_STUDIO_MAGIC, renderingSpeed: "QUALITY",
        });
        const dl = await ideogram.download(url);
        const ms = Date.now() - t0;
        console.log(`[studio-autoflow] Ideogram V3 ok in ${ms} ms (job ${jobId})`);
        jobs.set(jobId, { status: "done", image: `data:${dl.contentType};base64,${dl.buffer.toString("base64")}`, ms, engine: "ideogram-v3" });
      })().catch((e) => {
        console.error("studio auto-generate (ideogram) error:", e.message, e.response || "");
        jobs.set(jobId, { status: "error", error: "Generierung (Ideogram V3) fehlgeschlagen: " + e.message });
      });
      return true;
    }

    // ── Segmind-Modelle (Nano Banana / Seedream / FLUX): Referenz als öffentliche URL ──
    if (!SEGMIND_KEY) return sendError(res, 500, "Nano Banana Pro nicht konfiguriert — SEGMIND_API_KEY in Render setzen");
    const refId = putRefImage(pic.buffer, pic.mediaType);
    const proto = (req.headers["x-forwarded-proto"] || "https").split(",")[0].trim();
    const refUrl = `${proto}://${req.headers.host}/studio-ref/${refId}`;

    // Gewähltes Segmind-Modell (Default = Pro); unbekannte Werte → Default.
    const segModel = segmind.MODELS[b.model] ? b.model : segmind.DEFAULT_MODEL;
    const segLabel = segmind.MODELS[segModel].label;
    ...
    const t0 = Date.now();
    (async () => {
      const out = await segmind.generate({
        apiKey: SEGMIND_KEY,
        prompt: b.prompt,
        imageUrls: [refUrl],
        aspectRatio: "9:16",
        safetyTolerance: SEGMIND_SAFETY,
        outputFormat: "png",
        model: segModel,
      });
      let image;
      if (out.buffer) image = `data:${out.contentType || "image/png"};base64,${out.buffer.toString("base64")}`;
      else if (out.b64) image = `data:image/png;base64,${out.b64}`;
      else image = await downloadToDataUrl(out.url);
      const ms = Date.now() - t0;
      console.log(`[studio-autoflow] ${segLabel} ok in ${ms} ms (job ${jobId})`);
      jobs.set(jobId, { status: "done", image, ms, engine: segModel });
    })().catch((e) => {
      console.error(`studio auto-generate (segmind ${segModel}) error:`, e.message, e.response || "");
      jobs.set(jobId, { status: "error", error: `Generierung (${segLabel}) fehlgeschlagen: ` + e.message });
    }).finally(() => { refImages.delete(refId); });
    return true;
  });
  ...
  // ── Temporäres Referenzbild ÖFFENTLICH ausliefern, damit Segmind es per
  //    image_urls abholen kann. Bewusst OHNE Admin-Token (sonst käme Segmind nicht
  //    ran); Schutz = unrat-bare Zufalls-ID + kurze TTL, nur frisch hochgeladene
  //    Referenzbilder. ──
  router.get(/^\/studio-ref\/([a-f0-9]+)$/, (req, res, params) => {
    const r = refImages.get(params && params[0]);
    if (!r || r.exp < Date.now()) return sendError(res, 404, "Not found");
