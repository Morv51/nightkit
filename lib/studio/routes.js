...
function register(router) {
  ...
  // ── Auto-Flow (Beta): Ein-Flyer-Generierung via OFFIZIELLE OpenAI-Bild-API
  //    (GPT Image, gpt-image-1). Referenzbild + erzeugter Prompt → neues Template.
  //    Nutzt openaiImage.editImage (Edits-Endpunkt) mit input_fidelity="" →
  //    erstellt frei im Stil der Referenz (statt sie zu bewahren). Format folgt
  //    der Referenz (size="auto"). KEIN Ideogram, KEINE ChatGPT-Weboberfläche.
  //
  //    ASYNCHRON (gleiches Job-Muster wie /api/generate): sofort 202 + jobId,
  //    Generierung läuft im HINTERGRUND, das Frontend pollt /admin/auto-generate/
  //    <jobId>. So bleibt KEIN HTTP-Request 1–2 Minuten offen → der Render-Gateway
  //    kann ihn nicht mehr mit 502 kappen (genau das war die Ursache). ──
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    (async () => {
      const { b64 } = await openaiImage.editImage({
        ...
        inputFidelity: "", // CREATE: nicht die Referenz bewahren, sondern neu im Stil
      });
      const ms = Date.now() - t0;
      console.log(`[studio-autoflow] GPT Image ok in ${ms} ms (job ${jobId})`);
      jobs.set(jobId, { status: "done", image: `data:image/png;base64,${b64}`, ms, engine: "openai" });
    })().catch((e) => {
      console.error("studio auto-generate error:", e.message, e.response || "");
