...
function register(router) {
  ...
  //
  //    Ab Anzahl 2 geht der Prompt vorher durch den Variantencall (prompts/variants.txt
  //    als instructions). Jede Fassung wird eine eigene Kachel mit eigenem t.prompt.
  //    Bei Anzahl 1 entfaellt der Variantencall.
  //
  ...
  router.post("/admin/aprompt/autoflow", async (req, res) => {
    ...
    (async () => {
      try {
        ...
        if (anzahl > 1) {
          const v = await analysisPrompt.makeVariants({ apiKey: OPENAI_KEY, prompt, anzahl });
        ...
        jobs.set(jobId, {
          ...
          // ueberzaehlige Varianten sind harmlos — die ersten N gelten, der Rest wird verworfen
          // und hier vermerkt, damit es in der Maske sichtbar wird.
          verworfen: Math.max(0, geliefert - prompts.length),
