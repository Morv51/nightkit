...
function register(router) {
  ...
  // ── Auto-Varianten: N fertige Varianten-Prompts aus EINEM Stil-Anker (frei,
  //    kein externer Call). Variieren automatisch in Farbe/Illustration/Aufbau. ──
  router.post("/admin/auto-variants", async (req, res) => {
    if (!ok(req, res)) return true;
    const b = await body(req, res);
    if (!b) return true;
    if (!b.dna) return sendError(res, 400, "Stil-Anker fehlt — erst eine Referenz analysieren");
    const variants = promptBuilder.buildAutoVariants(b.dna, { refType: b.refType, count: b.count });
