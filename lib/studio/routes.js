...
function register(router) {
  ...
  router.post("/admin/redesign/family-roll", async (req, res) => {
    ...
    try {
      ...
      const out = await autoflowGen.redesignFamilyRoll({ images, model: b.model || "sonnet" });
      sendJson(res, 200, { ok: true, accent: out.accent, tone: out.tone, family_text: out.family_text });
  ...
  router.post("/admin/redesign/family-save", async (req, res) => {
    ...
    const b = await body(req, res); if (!b) return true;
    try {
      const saved = await families.save(b.name, { family_text: b.family_text, accent: b.accent, tone: b.tone });
