...
const ideogram = require("../ideogram");        // read-only: edit(), download(), layerizeText()
...
function register(router) {
  ...
  router.post("/admin/autoflow/prompt", async (req, res) => {
    ...
    try {
      ...
      // ZUSAETZLICH die Kachel-Prompts. Nur Kacheln, die einen eigenen prompt tragen —
      // das sind die Analyse-Prompt-Laeufe und die Redesign-Nachwuerfe. Klassische Flows
      // haben das Feld nicht, dort bleibt tiles leer und die Ansicht faellt auf files
      // zurueck. files selbst bleibt unveraendert.
      const tiles = (Array.isArray(state.tiles) ? state.tiles : [])
        .filter((t) => t && typeof t.prompt === "string" && t.prompt)
        .map((t) => ({ index: t.index, fnum: t.fnum, kind: t.kind, num: t.num, prompt: t.prompt }));
      sendJson(res, 200, { ok: true, runId: b.runId, regelwerk: !!state.regelwerk, hand: !!state.hand, files, tiles });
  ...
  //    Produktionsprompt als Auto-Flow-Lauf starten: 1 bis 10 Varianten, KEIN
  //    Referenzbild. Bewusst eine EIGENE Route statt /admin/autoflow/start, damit dessen
  //    Datei-Pflicht und die bestehenden Panels unberuehrt bleiben. Der Browser schickt
  //    nur Prompt und Anzahl, alle uebrigen Eckwerte setzt der Server.
  //
  //    Ab Anzahl 2 wird der Produktionsprompt SELBST die erste Kachel, und der
  //    Variantencall erzeugt nur die restlichen N-1 (prompts/variants.txt als
  //    instructions). Bei drei Varianten also: Kachel 1 unveraendert, Kachel 2 und 3
  //    je eine Fassung. Bei Anzahl 1 entfaellt der Variantencall ganz.
  //
  ...
  router.post("/admin/aprompt/autoflow", async (req, res) => {
    ...
    const anzahl = Math.max(1, Math.min(10, parseInt(b.anzahl, 10) || 1));

    // JOB-MUSTER, auch bei Anzahl 1: der Variantencall kann Minuten laufen, und ein
    // EINZIGES Antwortformat haelt die Maske einfach. Abgefragt wird ueber die
    // vorhandene, generische Route /admin/aprompt/status.
    const jobId = jobs.create(30 * 60 * 1000);
    sendJson(res, 202, { jobId, anzahl });

    (async () => {
      try {
        let prompts = [prompt];
        let verworfen = 0;
        // Anzahl 1 -> bisheriger Weg, KEIN Variantencall.
        if (anzahl > 1) {
          // Das ORIGINAL ist Kachel 1, der Call erzeugt darum nur N-1 Fassungen. Die
          // Pruefung auf zu wenige greift damit von selbst gegen N-1, ebenso das
          // mitwachsende Ausgabelimit.
          const v = await analysisPrompt.makeVariants({ apiKey: OPENAI_KEY, prompt, anzahl: anzahl - 1 });
          // EIN Aufruf, unabhaengig von der Anzahl — die Anzahl steckt im Ausgabelimit,
          // nicht in der Aufrufzahl. Der Richtpreis ist darum ein Mittelwert.
          usage.count("openai_variants");
          prompts = [prompt].concat(v.prompts);
          verworfen = Math.max(0, v.geliefert - v.prompts.length);
        }
        const r = await autoflowRunner.startRun({
          flow: "p", mode: "Analyse-Prompt (" + prompts.length + (prompts.length === 1 ? " Variante" : " Varianten") + ", 9:16 nativ)",
          textOnly: true, sourceName: "Analyse-Prompt",
          tiles: prompts.map((p) => ({ prompt: p })),
          imgModel: openaiImage.PROMPT_GEN_MODEL,
          imgSize: openaiImage.PROMPT_GEN_SIZE,
          // deescalate bleibt AUS: die Varianten sind so wenig umzuschreiben wie das Original.
          rawPrompt: true,
        });
        jobs.set(jobId, {
          status: "done", runId: r.runId, anzahl: prompts.length, angefordert: anzahl,
          // ueberzaehlige Fassungen sind harmlos — die ersten N-1 gelten, der Rest wird
          // verworfen und hier vermerkt, damit es in der Maske sichtbar wird.
          verworfen,
          model: openaiImage.PROMPT_GEN_MODEL, size: openaiImage.PROMPT_GEN_SIZE,
        });
      } catch (e) {
        const msg = e && e.message ? e.message : String(e);
        console.error("aprompt autoflow error:", msg);
        jobs.set(jobId, { status: "error", error: msg, tooFew: !!(e && e.tooFew) });
      }
    })();
    return true;
  });
  ...
  // ── DIAGNOSE: Text-Ebenen eines Bildes auslesen (Ideogram layerize-text) ──
  //    Schickt EIN Bild an POST /v1/ideogram-v3/layerize-text und gibt die Antwort
  //    UNVERAENDERT zurueck. Bewusst ohne jede Weiterverarbeitung: kein Parsen von
  //    text_blocks, keine Uebernahme in den Befuellschritt, keine Beruehrung
  //    bestehender Routen. Nur hinschicken und anzeigen.
  //
  //    Das dokumentierte Antwortschema nennt nur base_image_url, seed und
  //    original_image_url; die in der Beschreibung erwaehnten text_blocks stehen
  //    dort nicht. Genau darum wird nichts gefiltert — was kommt, kommt.
  //
  //    Hinweis zur Groesse: parseDataUrl deckelt bei 10 MB (Ideogram-edit-Limit),
  //    layerize-text erlaubt laut Doku 25 MB. Der engere Deckel gilt.
  router.post("/admin/layerize", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    if (!IDEOGRAM_KEY) return sendError(res, 500, "Ideogram nicht konfiguriert — IDEOGRAM_API_KEY in Render setzen");
    const pic = parseDataUrl(b.image);
    if (!pic) return sendError(res, 400, "Bild fehlt oder ist kein PNG/JPEG/WebP unter 10 MB");
    try {
      const r = await ideogram.layerizeText({
        apiKey: IDEOGRAM_KEY,
        imageBuffer: pic.buffer,
        imageType: pic.mediaType,
        prompt: b.prompt,
      ...
      // httpStatus mitgeben, damit auch ein 4xx/5xx von Ideogram sichtbar wird,
      // statt hier in einen eigenen Fehler uebersetzt zu werden.
      sendJson(res, 200, { ok: true, httpStatus: r.status, body: r.body, text: r.text });
    } catch (e) {
      const msg = e && e.message ? e.message : String(e);
      console.error("layerize error:", msg);
      sendError(res, 502, "layerize-text fehlgeschlagen: " + msg);
    }
    return true;
  });
}
