...
function register(router) {
  ...
  router.post("/admin/analyze", async (req, res) => {
    ...
    try {
      ...
      if (b.mode === "flyer") {
        ...
        // Build ist maskiert + nur Ersetzen → replaceOnly-Prompt.
        const prompt = promptBuilder.buildPlaceholderPrompt(zones, { replaceOnly: true });
  ...
  // ── Build-Prompt (nur Ersetzen, maskiert) aus Zonen neu bauen (frei) ──
  router.post("/admin/build-placeholder-prompt", async (req, res) => {
    if (!ok(req, res)) return true;
    const b = await body(req, res);
    if (!b) return true;
    const prompt = promptBuilder.buildPlaceholderPrompt(b.zones || [], { replaceOnly: true });
    return sendJson(res, 200, { prompt });
  });

  // ── Add-Prompt für einen einzelnen Platzhalter (Zielbox-Schritt), frei ──
  router.post("/admin/build-add-prompt", async (req, res) => {
    if (!ok(req, res)) return true;
    const b = await body(req, res);
    if (!b) return true;
    const prompt = promptBuilder.buildAddPrompt(b.role, {
      infoRef: b.infoRef || null,
      headlineRef: b.headlineRef || null,
    });
    return sendJson(res, 200, { prompt });
  });
  ...
  // ── Maskierter Edit (V3 Inpainting) — NUR der maskierte Bereich wird neu
  //    generiert, der Rest bleibt. Genutzt vom Build (nur Textzonen ersetzen)
  //    und vom Platzhalter-Einsetzen (gezogene Zielbox). magic_prompt OFF. Der
  //    pixelgenaue Composite passiert im Frontend. ──
  router.post("/admin/edit-masked", async (req, res) => {
    if (!ok(req, res)) return true;
    if (!IDEOGRAM_KEY) return sendError(res, 500, "IDEOGRAM_API_KEY not configured");
    const b = await body(req, res);
    if (!b) return true;
    if (!b.prompt) return sendError(res, 400, "Prompt fehlt");
    const pic = parseDataUrl(b.image);
    const mask = parseDataUrl(b.mask);
    if (!pic) return sendError(res, 400, "Bild fehlt oder ist ungültig");
    if (!mask) return sendError(res, 400, "Maske fehlt oder ist ungültig");
    try {
      const { url } = await ideogramV3.editMasked({
        apiKey: IDEOGRAM_KEY, prompt: b.prompt,
        imageBuffer: pic.buffer, maskBuffer: mask.buffer, imageType: pic.mediaType,
      });
      const dl = await ideogram.download(url);
      return sendJson(res, 200, { image: `data:${dl.contentType};base64,${dl.buffer.toString("base64")}` });
    } catch (e) {
      console.error("studio edit-masked error:", e.message, e.response || "");
      return sendError(res, 502, "Maskierter Edit fehlgeschlagen: " + e.message);
    }
  });
