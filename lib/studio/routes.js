...
function register(router) {
  ...
  router.post("/admin/analyze", async (req, res) => {
    ...
    try {
      ...
      const prompt = promptBuilder.buildMoodboardPrompt(dna, { refType, looseInspiration: !!b.loose, abstract: !!b.abstract });
  ...
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    (async () => {
      ...
      if (textOnly) {
        ...
        const out = await openaiImage.generateImage({ apiKey: OPENAI_KEY, prompt: sentPrompt, moderation: "low" });
