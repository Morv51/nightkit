...
function register(router) {
  ...
  //    der Referenz (size="auto"). Großzügiger Timeout steckt in editImage (180s).
  //    KEIN Ideogram, KEINE ChatGPT-Weboberfläche. Liefert Bild + reine Generier-
  //    Dauer (ms) zurück; Fehler im Klartext. ──
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    try {
      ...
      console.log(`[studio-autoflow] GPT Image ok in ${ms} ms`);
      return sendJson(res, 200, { image: `data:image/png;base64,${b64}`, ms, engine: "openai" });
    } catch (e) {
      console.error("studio auto-generate error:", e.message, e.response || "");
      return sendError(res, 502, "Generierung (GPT Image) fehlgeschlagen: " + e.message);
    }
  });
