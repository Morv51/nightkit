...
function register(router) {
  ...
  router.post("/admin/tag-one", async (req, res) => {
    ...
    try {
      const buf = templates.loadBuffer(file); // Sicherheitsgrenze + Existenzprüfung
      const tags = await vision.tagFlyer({ imageBase64: buf.toString("base64"), mediaType: mediaTypeFor(file), model: "sonnet" });
