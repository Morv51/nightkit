...
function register(router) {
  ...
  // Nur Prompts: je gewaehlter Vorlage EIN Sonnet-Aufruf, kein Bild. Gibt wie bei Artwork den
  // fertigen Bildprompt UND die rohe Sonnet-Antwort zurueck.
  router.post("/admin/redesign/preview", async (req, res) => {
    ...
    const items = [];
    // Dieselbe Reihum-Mechanik wie im Runner, Offset aus dem Vorschau-Moment. Vorschau und Lauf
    // streuen damit unabhaengig — die Zuordnung Template->Richtung ist je Moment eine andere,
    // die garantierte Vielfalt im Stapel ist dieselbe. direction geht mit in die Antwort.
