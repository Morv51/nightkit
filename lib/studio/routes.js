...
// Tatsaechliche Pixelmasse eines data-URL-Bildes (fuer die Dimensions-Anzeige des 9:16-Tests).
// Rein lesend, wirft nie; liefert { w, h, ratio } oder null.
async function measureDataUrl(dataUrl) {
  try {
    const p = parseDataUrl(dataUrl);
    if (!p || !p.buffer) return null;
    const meta = await require("sharp")(p.buffer).metadata();
    if (meta && meta.width > 0 && meta.height > 0) {
      return { w: meta.width, h: meta.height, ratio: Math.round((meta.width / meta.height) * 10000) / 10000 };
    }
  } catch (_) {}
  return null;
}
...
function register(router) {
  ...
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    const noFal = !!b.noFal; // Test-Weg "Nur GPT Image": den fal-Outpaint komplett ueberspringen
    ...
    (async () => {
      ...
      if (textOnly) {
        ...
        if (!noFal && openaiImage.GEN_SIZE === "1024x1536") { try { image = await outpaintTo916(image); } catch { /* 2:3 behalten */ } }
      } else {
      ...
      const dims = await measureDataUrl(image); // tatsaechliche Ausgabe-Masse (Kern des 9:16-Tests)
      console.log(`[studio-autoflow] ${textOnly ? "text->image" : "GPT Image"}${noFal ? " (ohne fal)" : ""} ok in ${ms} ms (job ${jobId})${dims ? " " + dims.w + "x" + dims.h : ""}`);
      jobs.set(jobId, { status: "done", image, ms, engine: "openai", dims });
    })().catch((e) => {
