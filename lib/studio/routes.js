...
function register(router) {
  ...
  router.post("/admin/auto-generate", async (req, res) => {
    ...
    (async () => {
      ...
      console.log(`[studio-autoflow] Nano Banana Pro ok in ${ms} ms (job ${jobId})`);
      jobs.set(jobId, { status: "done", image, ms, engine: "nano-banana-pro" });
    })().catch((e) => {
      console.error("studio auto-generate (segmind) error:", e.message, e.response || "");
      jobs.set(jobId, { status: "error", error: "Generierung (Nano Banana Pro) fehlgeschlagen: " + e.message });
