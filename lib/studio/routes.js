...
function register(router) {
  ...
  //    NUR PROMPTS: derselbe Variantencall wie beim Bildlauf, dieselbe Anzahl,
  //    aber KEIN Bild. Liefert die Liste zurueck — Kachel 1 der Produktionsprompt,
  //    danach die Fassungen. Job-Muster, weil der Textcall lange laeuft; abgefragt
  //    wird ueber dieselbe Route /admin/aprompt/status.
  //    /admin/aprompt/autoflow bleibt unberuehrt.
  router.post("/admin/aprompt/variants", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    if (!OPENAI_KEY) return sendError(res, 500, "OpenAI nicht konfiguriert — OPENAI_API_KEY in Render setzen");
    ...
    (async () => {
      try {
        let prompts = [prompt];
        let verworfen = 0;
        // Wie beim Bildlauf: das ORIGINAL ist der erste Eintrag, der Call
        // erzeugt darum nur N-1 Fassungen.
        if (anzahl > 1) {
          const v = await analysisPrompt.makeVariants({ apiKey: OPENAI_KEY, prompt, anzahl: anzahl - 1 });
          usage.count("openai_variants");
          prompts = [prompt].concat(v.prompts);
        ...
        jobs.set(jobId, { status: "done", prompts, anzahl: prompts.length, angefordert: anzahl, verworfen });
      } catch (e) {
        const msg = e && e.message ? e.message : String(e);
        console.error("aprompt variants error:", msg);
        jobs.set(jobId, { status: "error", error: msg, tooFew: !!(e && e.tooFew) });
      }
    })();
    return true;
