...
const promptgen = require("./promptgen");            // Prompt-Generator (Admin-Werkzeug, Beta)
...
function register(router) {
  ...
  // Bookmarkfaehige Route -> Reiter im Studio. Gleiches Muster wie /admin/manage.
  router.get("/admin/promptgen", async (req, res) => {
    res.setHeader("Location", "/admin/template-studio#promptgen");
    res.setHeader("Cache-Control", "no-cache");
    res.writeHead(302);
    res.end();
    return true;
  ...
  // Verfuegbarkeit: kostenlos, kein Modellaufruf. Schaltet den Reiter frei (gleiches Muster
  // wie manage-panel.js) und meldet vorab, ob der Anthropic-Schluessel gesetzt ist.
  router.get("/admin/promptgen/status", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    sendJson(res, 200, { ok: true, ready: vision.isConfigured() });
    return true;
  });

  // Call 1: Bild + optionaler Kontext -> Masteranalyse. Das Bild wird NICHT gespeichert,
  // es geht nur durch diesen Aufruf. Die Antwort traegt das Ergebnis der serverseitigen
  // Abschnittspruefung mit; fehlt etwas, startet die Oberflaeche Call 2 gar nicht erst.
  router.post("/admin/promptgen/analyze", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    ...
    if (!pic) return sendError(res, 400, "Bild fehlt oder ist ungueltig (PNG/JPEG/WebP, max 10 MB)");
    try {
      const out = await promptgen.runAnalysis({
        imageBase64: pic.base64, mediaType: pic.mediaType,
        userContext: b.context, model: b.model || "sonnet",
      });
      sendJson(res, 200, { ok: true, ...out });
    } catch (e) { sendError(res, 502, "Analyse fehlgeschlagen: " + (e && e.message ? e.message : e)); }
    return true;
  });

  // Call 2: Masteranalyse -> Produktionsprompt. BEWUSST ohne Bild: die Analyse ist
  // verbindlich, hier wird nicht neu interpretiert. Das Abschnitts-Gate greift auch hier
  // nochmal, damit ein direkter Aufruf es nicht umgehen kann.
  router.post("/admin/promptgen/convert", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    if (!vision.isConfigured()) return sendError(res, 500, "ANTHROPIC_API_KEY not configured");
    try {
      const out = await promptgen.runConversion({ analysis: b.analysis, model: b.model || "sonnet" });
      sendJson(res, 200, { ok: true, ...out });
    } catch (e) {
      if (e && e.status === 400) return sendError(res, 400, e.message);
      sendError(res, 502, "Konvertierung fehlgeschlagen: " + (e && e.message ? e.message : e));
    }
    return true;
  });
