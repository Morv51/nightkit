...
function postMultipart({ apiKey, fields, files }) {
...
// Modus 2 (Engine-Option Ideogram): REINE Text-zu-Bild-Generierung OHNE
// Style-Reference. Der Stil steckt komplett im Prompt-Text — das Moodboard wird
// NICHT als Referenzbild durchgereicht (das hatte den Inhalt 1:1 kopiert).
// magic_prompt OFF, aspect_ratio 9x16, höchste Qualität.
function generate({ apiKey, prompt, aspectRatio = "9x16", magicPrompt = "OFF", renderingSpeed = "QUALITY" }) {
  if (!prompt) return Promise.reject(new Error("Prompt missing"));
  return postMultipart({
    apiKey,
    fields: {
...
module.exports = { generateWithStyleRef, generate };
