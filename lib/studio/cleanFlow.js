...
const ARTWORK_3 =
  "One bold move carries the poster: the headline is huge, runs in a single line and bleeds off both edges, " +
  "and the subject passes in front of it so whole letters disappear behind the image. Dense in one area, close " +
  "to empty in another. The smaller texts follow the same craft: clear size hierarchy between the groups, " +
  "never a flat even list. Headline horizontal.";
// Traeger-Regel steht hier ARTWORK-EIGEN (die gemeinsame P_TYPO_DEVICES bleibt Nachbau/Variante
// vorbehalten): einzeilige Felder duerfen auf einem der physischen Objekte sitzen, der DJ-Block
// und andere mehrzeilige Gruppen nie.
const ARTWORK_5 =
  "The poster is assembled from physical matter, not rendered: torn paper, tape, stickers and labels with real " +
  "edges and shadows, paper grain, print texture, slight misregistration. A single-line text may sit on one of " +
  "these objects. Multi-line groups such as the DJ names never do.";
...
function buildCleanArtworkPrompt(spec) {
  ...
  const colour = cw
    ? "Colour world: " + cw + ". One gesture in this colour at poster scale, a single large shape or physical " +
      "object. Never thin lines or small accents."
    : "";
