...
// ── REDESIGN-Modus: Eingabe ist ein FERTIGES Template aus dem Bestand. Foto, Komposition und
// Farbwelt bleiben, NUR die Textsetzung wird neu gemacht und die Grafikebene ergaenzt. Darum:
// KEIN staging (die Komposition steht schon), KEINE palette (die Farbwelt bleibt), KEIN family.
// Sonnet liefert genau EIN Objekt je Vorlage mit sechs Feldern.
//
// Zwei bewusste Abweichungen von Artwork:
//   • Das Format ist die KURZE Zeile "Format exactly 9:16 vertical." statt P_FORMAT_9x16 — dessen
//     Erweiterungs-Satz ("extend the design naturally to fill 9:16") waere hier falsch, weil das
//     Bild unveraendert bleiben soll.
//   • P_LITERAL_ARTWORK wird geteilt (gleiche Wortlaut-Anforderung, gleiche Wortliste).
// Fehlende Felder verschlechtern die Regie, nehmen aber nie Eventinformation: die Platzhalter-
// Namen und die festen Regeln stehen immer, es faellt nur die Positionsangabe weg (place()).
const P_FORMAT_9x16_KURZ = "Format exactly 9:16 vertical.";

function buildCleanRedesignPrompt(spec) {
  const r = spec || {};
  const hs = v(r.headline_style), carriers = v(r.carriers);
  return [
    "Redesign the typography of this club poster. Keep the photograph, the composition and the colour " +
      "world exactly as they are. Nothing about the image itself changes.",
    "Clear the existing text setting completely, including any bars, panels or shapes it sat on, and set " +
      "the same information again as a designer would.",
    // headline_style sitzt in der Typografie-Zeile; fehlt es, bleibt der Charakter-Satz stehen.
    "The headline is by far the largest element and stays complete and fully readable. " +
      (hs ? "It is " + hs + " and carries all the typographic character. " : "It carries all the typographic character. ") +
      "Everything else is set in a quiet neutral sans, light weight, uppercase, with wide letter-spacing, " +
      "always small.",
    "Four size steps, each clearly smaller than the one before: headline, then the DJ names and CLUBNAME, " +
      "then LOCATION and the website, then the graphic layer.",
    "The four groups are never set alike and sit far apart, each in its own area. Never two of them stacked " +
      "in the same column. No group floats in open space: every one is anchored to an edge, a bar or a rule.",
    ["The groups:",
      place("HEADLINE with SUBLINE", r.place_headline,
        "the subline set small directly underneath, never approaching the headline size"),
      place("19.06.26 with UHRZEIT", r.place_date),
      place("DJ NAME 1, DJ NAME 2, DJ NAME 3", r.place_lineup,
        "all three at exactly the same size and weight, no numbers"),
      place("CLUBNAME", r.place_club,
        "as one single line, with LOCATION and www.website.com directly under it, one step smaller"),
    ].join("\n"),
    // carriers sitzt am Ende des Traeger-Absatzes; fehlt es, bleibt der ganze Absatz gueltig.
    "A carrier is a small physical object a single line of text sits on: torn paper, tape, a tag, a label, " +
      "a solid bar. At most two groups get one, and only single lines. Multi-line groups sit directly on the " +
      "artwork. Every carrier stays small and never becomes a second focal point. Carriers come from the " +
      "world of the photograph." + (carriers ? " For this poster: " + carriers + "." : ""),
    "Build a secondary layer of print furniture: registration crosses, corner ticks, hairline rules, small " +
      "solid bars, arrows, asterisks, dot grids. It lives in the calm areas, never on top of busy " +
      "photographic detail. Every mark is crisp and sharp, never soft or blurred. It does not avoid the " +
      "text: each group is built into it, framed by rules, brackets or a bar. Always a clear step smaller " +
      "than the smallest placeholder.",
    "The poster has no frame, no border, no cut-out or perforated outline.",
    WORDLIST + " Set them at the size of the graphic layer. " + NOTHING_ELSE,
    P_FORMAT_9x16_KURZ,
    P_LITERAL_ARTWORK,
  ].filter(Boolean).join("\n\n");
