...
function buildCleanRedesignPrompt(spec) {
  ...
  const words = pickWords(r.words);

  return [
    "Rebuild this club poster as one composition. Keep the subject and the colour world, everything else " +
      "may be restaged.",
    staging ? staging : "",
    // Headline. Der STYLE-Zweig ist der Normalfall: die Buchstabenformen tragen den Charakter.
    // Der Material-Zweig kommt NUR dazu, wenn headline_material gesetzt ist — er ersetzt den
    // Style-Zweig nicht. Die Verschraenkung mit dem Motiv gilt in beiden Faellen.
    ["The headline is huge, runs in one line and is set tight so the letters almost touch." +
      ...
      "All letters share one common baseline and one common cap height: no letter jumps, dances or " +
        "sits higher or lower than its neighbours.",
      hs ? "The letterforms are " + hs + " and they alone carry the character of the poster." : "",
      material
        ...
        : "",
      // Nennt das Token HEADLINE woertlich — sonst kommt der Platzhalter im ganzen Absatz nicht
      // mehr vor und das Modell erfindet eine Schlagzeile (dieselbe Falle wie in Geruest B).
    ...
    ["The groups:",
      ...
      place("DJ NAME 1, DJ NAME 2, DJ NAME 3", lc(r.place_lineup),
    ...
    [words.length === 2
      ? "Besides the placeholders, the only other words on the poster are " + words[0] + " and " +
        words[1] + ", placed far apart, set bare, never inside drawn badges, tickets, circles or frames."
      : words.length === 1
      ? "Besides the placeholders, the only other word on the poster is " + words[0] +
        ", set bare, never inside drawn badges, tickets, circles or frames."
      : "",
      NOTHING_ELSE].filter(Boolean).join(" "),
    P_FORMAT_9x16_KURZ,
    P_LITERAL_ARTWORK,
  ].filter(Boolean).join("\n\n");
}
