...
// Ersetzt die Zaehlregel "covers at most two letters": gezaehlt hat das Modell nie, und die
// Regel liess zu, dass die MASSE des Motivs Buchstaben schluckt. Jetzt zaehlt die Dicke.
// GETEILT (Artwork + Redesign): die Regel ist modus-neutral, eine Entkopplung hiesse, die
// als kaputt erkannte Fassung in Artwork absichtlich zu behalten.
const H_THIN_CROSS =
  "Only thin parts of the subject cross the letters, a hand, hair, a stick, an edge. The word " +
  "never loses letters behind the subject's mass.";
const H_INTERLOCK =
...
function headlineParagraph({ place, style, material, flatContrast }) {
  return [
    ...
    material
      ...
      // flatContrast: auch der flache Zweig sichert die Lesbarkeit ueber Kontrast. GETEILT, per
      // Nutzer-Entscheid: der fehlende Kontrast ist in beiden Modi derselbe Fehler. Der Wert ist
      // die QUELLANGABE, weil der Referent sich unterscheidet — Redesign zieht aus dem Original-
      // Poster, Artwork aus der Regie-Palette, die im Geruest selbst benannt ist.
      : (flatContrast
        ? "Its colour is chosen for maximum contrast against the ground it crosses, drawn from " +
          flatContrast + "."
        : ""),
    H_READABLE,
    H_THIN_CROSS,
...
// REDESIGN-EIGEN (Groessen-Fix): vier benannte Stufen mit fester Zugehoerigkeit, damit keine
// Textgruppe unter Telefon-Lesbarkeit faellt. P_HIERARCHY bleibt fuer Artwork unangetastet — die
// Groessen-Regie ist redesign-spezifisch, darum getrennt statt geteilt geaendert.
const REDESIGN_HIERARCHY =
  "Hierarchy, four fixed steps. The headline rules the poster. Second voice, strong and present: the " +
  "DJ names, CLUBNAME and SUBLINE. Third step: the date, UHRZEIT, LOCATION and the website, small but " +
  "comfortably legible on a phone screen, never caption size. Fourth: the graphic layer and the two " +
  "words, smaller than all other text. No text anywhere falls below phone legibility.";
...
// REDESIGN-EIGEN: "small" statt "tiny", damit die Uhrzeit nicht unter Telefon-Lesbarkeit faellt.
// DATE_STACK bleibt fuer Artwork unveraendert.
const REDESIGN_DATE_STACK =
  " split into three stacked parts, 19 above 06 above 26, set heavy, tight leading, a thin vertical " +
  "rule at its left, UHRZEIT small at its foot.";
const P_ANTI_SYSTEM =
...
// size (nur Geruest A): haengt die Groessenangabe an. anchored (nur Redesign): verankert die zwei
// Woerter an der Datums- bzw. Club-Gruppe. Beide sind Schalter, damit je Modus genau EINE
// Fassung entsteht — der jeweils andere Modus bleibt zeichengleich.
function wordsLine(raw, opts) {
  ...
  // anchored (nur Redesign): die zwei Woerter sind an Gruppen verankert statt frei in der Hoehe.
  const anker = (opts && opts.anchored)
    ? " One sits near the date group, one near the club block, each a clear step away from its " +
      "group, never in the top corners."
    : "";
  ...
  if (w.length === 2)
    return "Besides the placeholders, the only other words on the poster are " + w[0] + " and " +
      w[1] + ", placed far apart" + nackt + anker + size;
  if (w.length === 1)
...
function buildCleanRedesignPrompt(spec) {
  ...
  return [
    "Rebuild this club poster as one composition. Keep the subject and the colour world, everything else " +
      "may be restaged.",
    staging ? staging : "",
    headlineParagraph({ place: ph, style: hs, material, flatContrast: "the original's palette" }),
    REDESIGN_HIERARCHY,
    ...
    ["The groups:",
      "19.06.26" + (pd ? " sits " + pd + "," : "") + REDESIGN_DATE_STACK,
      place("DJ NAME 1, DJ NAME 2, DJ NAME 3", lc(r.place_lineup), DJ_RULE),
      // Groessen fest an die Stufen gebunden: CLUBNAME = zweite Stimme, LOCATION/Website = dritte.
      place("CLUBNAME with LOCATION and www.website.com", lc(r.place_club),
        "CLUBNAME one line at second-voice size, LOCATION and www.website.com under it at the third " +
        "step, wide letter-spacing"),
      // "flush to the end" scheiterte, wenn die Headline bis an die rechte Kante laeuft — dann ist
      // neben ihrem Ende kein Platz. Unter den letzten Buchstaben geht immer. Groesse gebunden.
      "SUBLINE sits directly under the final letters of the headline, ranged right, at second-voice " +
        "size, joined by a hairline.",
    ].join("\n"),
    ...
    // Traeger-Ausnahme: der gestapelte Datumsblock darf trotz Ein-Zeilen-Regel auf einem schmalen
    // vertikalen Streifen sitzen — so wird der Dreierstapel/Ein-Zeile-Konflikt zur Regel.
    carriers ? "Carriers: " + carriers + ". A carrier holds one single line, or the stacked date on a " +
      "narrow vertical strip. It stays small and never becomes a second focal point." : "",
    (!field && !carriers)
    ...
    // Akzentfarbe sichern: die Farbwelt bleibt beim Redesign erhalten, aber ohne diesen Satz
    // kam die Akzentfarbe des Originals in der NEUEN Textsetzung oft nicht mehr vor.
    "The original poster's accent colour appears in the new type setting: SUBLINE or one other " +
      "small group carries it.",
    ...
    // anchored: die zwei Woerter an Datums- und Club-Gruppe verankert, nie in den oberen Ecken.
    [wordsLine(r.words, { anchored: true }), NOTHING_ELSE].filter(Boolean).join(" "),
    P_FORMAT_9x16_KURZ,
    P_LITERAL_ARTWORK,
  ].filter(Boolean).join("\n\n");
}
