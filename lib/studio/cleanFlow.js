...
// EIGENER kurzer Kreativ-Brief. Er ersetzt den frueheren Artwork-Koerper KOMPLETT: KEIN
// P_COMPOSITION (Motiv-Schutz/free-zones widersprechen dem gewollten "subject cuts across the
// headline, depth not overlay"), KEIN P_TYPO_A (Headline/Nebentext-Regel steht jetzt inline),
// KEINE Traeger-Regel, KEINE DJ-Nummerierung, KEINE Logo-Entfernung. Diese Bausteine bleiben als
// geteilte Konstanten fuer Nachbau/Variante UNANGETASTET; Artwork nimmt sie nur nicht mehr auf.
// UNVERAENDERT beibehalten: P_PLACEHOLDERS (inkl. 19.06.26), P_FORMAT_9x16, P_LITERAL. Farbwelt
// steht inline in Absatz 4 (Sonnet liefert weiterhin nur label + color_world).
const ARTWORK_1 = "Build a club poster from the attached photo.";
const ARTWORK_2 =
  "Treat the photo as raw material, not as a finished background. Crop it hard, scale it far past the frame, " +
  "let it bleed off the edges, push the tonality.";
// "Dense in one area, close to empty in another" ist RAUS: in zwei Laeufen ohne Wirkung. Die
// Leere entsteht jetzt konkret ueber die Groesse — alles ausser der Headline steht klein und
// leicht, dadurch wirkt die Headline monumental.
const ARTWORK_3 =
  "One bold move carries the poster: the headline is huge, runs in a single line and bleeds off one edge, and " +
  "the subject passes in front of it so whole letters disappear behind the image. Everything else is set small, " +
  "at fine print scale and in a light weight, so the headline reads as monumental against it. Two or three " +
  "steps of size within that small type. All text sits inside one consistent margin. Headline horizontal.";
// Traeger-Regel steht hier ARTWORK-EIGEN (die gemeinsame P_TYPO_DEVICES bleibt Nachbau/Variante
// vorbehalten) und ist bewusst eng: HOECHSTENS EIN Textfeld im ganzen Plakat sitzt auf einem
// Objekt, der Rest steht direkt auf dem Artwork. Der Zusatz gegen Sammel-Flaechen verhindert,
// dass mehrere Felder in einer gemeinsamen Form/Box zusammengefasst werden.
const ARTWORK_5 =
  "The poster is assembled from physical matter, not rendered: torn paper, tape, stickers and labels with real " +
  "edges and shadows, paper grain, print texture, slight misregistration. At most one single text field in the " +
  "whole poster sits on such an object. All other text sits directly on the artwork, and no group of fields is " +
  "ever collected into one shape or panel.";

// Artwork-Bildprompt. spec: { color_world } (PFLICHT; der Aufrufer filtert Specs ohne Farbwelt).
function buildCleanArtworkPrompt(spec) {
  const cw = v((spec || {}).color_world);
  const colour = cw
    ? "Colour world: " + cw + ". One gesture in this colour at poster scale, a single large shape or physical " +
      "object. Never thin lines or small accents, and it carries no text."
    : "";
  return [ARTWORK_1, ARTWORK_2, ARTWORK_3, colour, ARTWORK_5, P_PLACEHOLDERS, P_FORMAT_9x16, P_LITERAL]
    .filter(Boolean).join("\n\n");
}
