...
// size (nur Geruest A): haengt die Groessenangabe an. spread (nur Redesign, Groessen-Fix): die zwei
// Woerter sitzen auf verschiedenen Hoehen, nicht gespiegelt in den oberen Ecken. Beide sind
// Schalter, damit je Modus genau EINE Fassung entsteht — der jeweils andere Modus bleibt
// zeichengleich.
function wordsLine(raw, opts) {
  ...
  const spread = (opts && opts.spread) ? ", at different heights, never mirrored in the two top corners" : "";
  ...
  if (w.length === 2)
    return "Besides the placeholders, the only other words on the poster are " + w[0] + " and " +
      w[1] + ", placed far apart" + spread + nackt + size;
  if (w.length === 1)
...
function buildCleanRedesignPrompt(spec) {
  ...
  return [
    ...
    // spread: die zwei Woerter auf verschiedenen Hoehen, nicht in den oberen Ecken gespiegelt.
    [wordsLine(r.words, { spread: true }), NOTHING_ELSE].filter(Boolean).join(" "),
    P_FORMAT_9x16_KURZ,
    P_LITERAL_ARTWORK,
  ].filter(Boolean).join("\n\n");
}
