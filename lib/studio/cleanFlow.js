...
function keepVariant(spec) {
  ...
  // NUR der modus-spezifische Kern (Motiv-Schutz + Komposition stehen im gemeinsamen P_COMPOSITION).
  const kopf =
    "This is a VARIATION in a matching design family: keep the original's vibe, its fonts and typographic treatment, and its handling of the graphic elements, but take the colour world, the person/motif and the arrangement in the new direction below.";
  return deltas.length ? kopf + "\n" + deltas.join("\n") : kopf;
}

// Varianten-Bildprompt: Nachbau-Geruest, nur der KEEP-Kern ersetzt. spec: { color_world, imagery_style, layout }.
