...
// Absatz 2, Nachbau: exakt gleiches Design, Farbe und Komposition erhalten.
...
// Nachbau-Prompt (unveraendert zur bisherigen Fassung).
const CLEAN_PROMPT = [P_INTRO, P_KEEP_NACHBAU, P_PLACEHOLDERS, P_TYPO, P_FORMAT, P_LITERAL].join("\n\n");

// Absatz 2, Variante: tauscht den "keep exact same colours/composition"-Satz (der einer Variante
// widerspraeche) gegen die Varianten-Formulierung und haengt die drei Sonnet-Vorgaben an. Vibe,
// Fonts, typografische Sorgfalt und der Umgang mit den Grafikelementen bleiben; Farbe, Person/Motiv
// und Anordnung wechseln. Leere Vorgaben werden weggelassen (kein toter Aufzaehlungspunkt).
function keepVariant(spec) {
  ...
  if (v(s.layout)) deltas.push("- Arrangement: " + v(s.layout));
  const kopf =
    "This is a VARIATION in a matching design family: keep the original's vibe, its fonts and typographic treatment, and its handling of the graphic elements, but take the colour world, the person/motif and the arrangement in the new direction below. Keep the main subject and all graphic elements whole and never covered by text. The text must sit in the free zones and complement the graphics — text and image form ONE composition.";
  return deltas.length ? kopf + "\n" + deltas.join("\n") : kopf;
}

// Varianten-Bildprompt: Nachbau-Geruest, nur Absatz 2 ersetzt. spec: { color_world, imagery_style, layout }.
function buildCleanVariantPrompt(spec) {
  return [P_INTRO, keepVariant(spec), P_PLACEHOLDERS, P_TYPO, P_FORMAT, P_LITERAL].join("\n\n");
}
