...
async function listRuns() {
  ...
  const runs = grouped.map((g) => {
    ...
    // Bauteil A.3: Fehlertyp unterscheiden. errKind kommt aus dem Runner (classifyError); fuer
    // ALTE Zustaende ohne errKind gilt status "blocked" als safety (so wie isBlocked es setzte).
    const KIND_LABEL = { safety: "Vom Bildmodell abgelehnt (Motiv): ", limit: "Guthaben/Limit: ", error: "" };
    const fileErrors = [];
    for (const f of files) {
      ...
      for (const t of tiles) {
        if (t.fnum === f.fnum && (t.status === "error" || t.status === "blocked") && t.reason) {
          const kind = t.errKind || (t.status === "blocked" ? "safety" : "error");
          kinds.add(kind);
          msgs.push((KIND_LABEL[kind] || "") + String(t.reason));
        }
      }
      if (msgs.length) {
        const kind = kinds.has("safety") ? "safety" : kinds.has("limit") ? "limit" : "error";
        fileErrors.push({ fnum: f.fnum, name: f.name || "", kind, text: msgs.join(" · ") });
      }
    }
    return {
      runId: g.runId,
