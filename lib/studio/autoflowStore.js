...
async function listRuns() {
  ...
  for (const key of keys) {
    ...
    if (!byRun.has(runId)) byRun.set(runId, { runId, images: [], adopted: new Map() });
  ...
  const runs = grouped.map((g) => {
    ...
    return {
      ...
      images: g.images.map((im) => ({ ...im, adopted: g.adopted.has(im.index) })),
    };
  });
...
module.exports = {
  ...
  getResultImage, getAdoption, setAdoption,
};
