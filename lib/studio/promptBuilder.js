...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  const lines = [
    ...(loose ? [
      "LOOSE INSPIRATION MODE (important — this OVERRIDES the style-matching instructions further below): Use " +
        "the reference image ONLY as loose stylistic inspiration for the GENERAL direction — overall mood, " +
        "genre, rough colour feel (as a principle, not exact) and general typographic character. Do NOT " +
        "reproduce its composition, layout, specific imagery, motif or arrangement. Create an ORIGINAL, " +
        "INDEPENDENT flyer design that is clearly DISTINCT from the reference — the same broad vibe, but a " +
        "different execution and a different layout, recognisable as its own work and NOT a rebuild of the " +
        "reference. Wherever an instruction below says to match the reference 'as closely as possible', to use " +
        "its 'exact' colour world / background, or to keep its composition, treat that as SOFTENED: keep only " +
        "the broad stylistic vibe and deliberately DIVERGE in composition, layout, colour balance and motif.",
    ] : []),
