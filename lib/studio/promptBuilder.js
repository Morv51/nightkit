...
function typoElementLines(elements, placeholders) {
  ...
  for (const el of Array.isArray(elements) ? elements : []) {
    ...
    if (v(el.effect) && v(el.effect).toLowerCase() !== "none") parts.push("and ONLY LAST a " + v(el.effect) + " effect/colour applied on top, without overriding the letterforms");
...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  const lines = [
    ...
    "COLOUR DISCIPLINE (fixed — this OVERRIDES the colour count above): use AT MOST TWO spot colours for the " +
      "whole flyer — the two DOMINANT colours taken from the analysed colour world above — PLUS black and " +
      "paper-white. Even if the reference shows more hues, reduce everything to these two dominant spot colours; " +
      "a photograph appears as a two-colour (duotone) print in those colours, keeping its print texture. NO " +
      "extra hues, no rainbow, no variety of gradients.",
    ...
    "PRINTED-FLYER REALISM (ALWAYS, regardless of the reference — the goal is a PRINTED flyer, not a rendered " +
      "3D image): use MATTE materials instead of plastic shine; NO glossy specular highlights on clothing, " +
      "faces or complexion; add a subtle print halftone / dot screen and light grain; use muted, restrained " +
      "colour transitions instead of saturated glowing gradients; and NO symmetrical sparkle or star-glint " +
      "shapes. Keep the whole thing looking like ink printed on paper. This never overrides the photographic-" +
      "or-illustrated decision above — it only removes the over-rendered, glossy CGI look.",
    "PRINT REGISTRATION & HALFTONE (fixed): show a VISIBLE halftone dot screen across the colour areas, and a " +
      "slight colour MISREGISTRATION between the printing inks — the colours offset by a hair, as in imperfect " +
      "offset printing. The whole flyer should look like a PRINTED flyer that was then SCANNED, not a clean " +
      "digital render.",
    ...
    "EVERY WORD COMPLETE AND READABLE (essential — the placeholder words must stay fully readable so they can " +
      "be replaced later): every word stays COMPLETE and legible; never drop, lose or split whole letters, and " +
      "never show a partial fragment like 'DLIN' instead of the full word. A large text element MAY be set " +
      "oversized so its stroke edge bleeds off the canvas (see BLEED below), but even then EVERY LETTER of the " +
      "word stays horizontally complete and readable.",
    "BLEED — CROP AT THE EDGE (do this on at least ONE large text element): at least one large text element " +
      "(typically the oversized headline) deliberately runs OFF the canvas edge and is cropped there, like on a " +
      "real designer's flyer — do NOT leave an even, safe margin all the way around. The bleed trims ONLY the " +
      "outer edge of the lettering (its top, bottom or one side); it NEVER removes a whole letter, so the word " +
      "itself still reads complete.",
  ...
  lines.push(
    ...
    "SHARED ALIGNMENT AXIS: the secondary texts (date, time, location, DJ names, clubname, website) SHARE a " +
      "common alignment edge — they line up along the SAME left (or right) edge or a common grid line, not " +
      "scattered freely around the canvas. One consistent alignment axis holds the secondary info together.",
