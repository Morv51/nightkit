...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  const lines = [
    ...
    "It is ONE single, cohesive club event flyer with ONE unified composition and a nightlife / club " +
      "aesthetic — mood, typography and imagery all read as a club/party event.",
    ...
    ...(isVariant ? [
      "If a previously generated flyer from this style family is available (e.g. earlier in this chat), use " +
        "it as the primary visual reference for consistency. Keep the same overall style, typography " +
        "treatment and level of restraint as that flyer — only change the color combination, the " +
        "illustration motif and the arrangement as specified below. If no such flyer exists, fall back to " +
        "the moodboard / reference image above.",
    ] : []),
    refLine,
    "STYLE GUIDELINE: " + style + ".",
    "Background: a single light/colored ground (matching the reference's background style), NOT black — " +
      "unless the reference is truly dark.",
    "Palette: minimal — one background color plus only one or two accent colors total. Restrained, not " +
      "multicolored. Do NOT use many bright colors at once.",
  ];
  ...
  if (ov.length) {
    ...
    lines.push("VARIATION — keep the STYLE ANCHOR identical (same mood, typography character, texture, " +
      "hand-drawn line/drawing style and overall composition logic); change ONLY this: " + ov.join("; ") +
      ". Still ONE big motif + text around it + lots of negative space — no grid, no collage.");
  }
...
// ── Auto-Varianten: N fertige Prompts aus EINEM Stil-Anker, automatisch
//    verschieden in Farbe / Illustration / Aufbau. Der Anker (Stimmung, Typo,
//    Textur, Zeichenstil, Kompositionslogik) bleibt fix; nur diese drei
//    Dimensionen variieren — deterministisch durchgereiht (kein Zufall nötig). ──
const AV_BACKGROUNDS = [
  "soft sky blue", "pale lime green", "warm olive", "dusty rose", "off-white cream",
  "muted mustard", "chalky mint", "faded terracotta", "pale periwinkle", "soft sage",
];
const AV_ACCENTS = [
  "warm olive", "burnt orange", "deep teal", "dusty rose", "charcoal grey",
  "ochre yellow", "brick red", "forest green", "faded navy", "plum",
];
const AV_MOTIFS = [
  { en: "a dog", de: "Hund" }, { en: "a cat", de: "Katze" }, { en: "a flower", de: "Blume" },
  { en: "a bird", de: "Vogel" }, { en: "a fish", de: "Fisch" }, { en: "a sun", de: "Sonne" },
  { en: "a potted plant", de: "Pflanze" }, { en: "a smiley face", de: "Gesicht" },
  { en: "a star", de: "Stern" }, { en: "an abstract squiggle", de: "Kritzel" },
  { en: "a heart", de: "Herz" }, { en: "a mushroom", de: "Pilz" },
  { en: "a cloud", de: "Wolke" }, { en: "a little hand", de: "Hand" },
];
const AV_LAYOUTS = [
  { en: "one big motif centered, the text wrapped around it", de: "Motiv mittig" },
  { en: "one big motif in the top half, the text grouped below it", de: "Motiv oben" },
  { en: "one big motif in the bottom half, the text grouped above it", de: "Motiv unten" },
  { en: "one big motif in a corner, the headline in the opposite area", de: "Motiv Ecke" },
  { en: "one big motif large and off-center, the text set into the negative space", de: "Motiv versetzt" },
];

function buildAutoVariants(dna, opts = {}) {
  const refType = opts.refType;
  const n = Math.max(1, Math.min(24, parseInt(opts.count, 10) || 10));
  const out = [];
  for (let i = 0; i < n; i++) {
    const bg = AV_BACKGROUNDS[i % AV_BACKGROUNDS.length];
    const acc1 = AV_ACCENTS[(i + 2) % AV_ACCENTS.length];
    const two = i % 2 === 0; // mal eine, mal zwei Akzentfarben
    const acc2 = AV_ACCENTS[(i + 5) % AV_ACCENTS.length];
    const accents = two ? `${acc1} and ${acc2}` : acc1;
    const colorWorld =
      `a light ${bg} background with only ${accents} as accent${two ? "s" : ""} — muted, chalky, ` +
      "desaturated and restrained (NOT bright, NOT saturated), light ground, not black";
    const motif = AV_MOTIFS[i % AV_MOTIFS.length];
    const imagery =
      `${motif.en} as the single big subject, drawn in the SAME loose childlike hand-drawn single-line ` +
      "doodle style as the reference — keep the exact crude/wobbly line style, change ONLY the subject";
    const layout = AV_LAYOUTS[i % AV_LAYOUTS.length];
    const prompt = buildMoodboardPrompt(dna, {
      refType,
      variant: { color_world: colorWorld, imagery_style: imagery, layout: layout.en },
    });
    out.push({ label: `${bg} + ${acc1} · ${motif.de} · ${layout.de}`, prompt });
  }
  return out;
}
