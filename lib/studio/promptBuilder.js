...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  lines.push(
    ...
    "ONLY THESE TEXTS: render ONLY the placeholder texts listed above and NO other text. Do NOT invent, add " +
      ...
      "placeholders.",
    "HEADLINE TEXT IS LITERAL: the headline text must be EXACTLY the placeholder word \"HEADLINE\" — nothing " +
      "else. Do NOT replace it with a thematic title, an event name or a festival name, and do NOT add, " +
      "prepend or append any extra word (no 'EVENT', no 'FESTIVAL', no names, no year, no tagline — e.g. never " +
      "'HOLI FESTIVAL' or 'EVENT HEADLINE', only the single literal word HEADLINE). The same applies to EVERY " +
      "field: render ONLY the literal placeholder words exactly as listed above, with no other, added, " +
      "translated or altered words anywhere on the flyer.",
