...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  // Auto-Flow 2 Pfad B: rein abstrakte Text-Beschreibung, KEIN Bild → generischere
  // Direktive + reduzierter Stil (nur grobe Essenz, keine konkreten Bildelemente).
  ...
  const style = (abstract
    // Pfad B: nur die grobe Essenz (Stimmung/Genre, Farb-Stimmung, Typo-Charakter) —
    // KEINE konkreten Bildelemente/Motive/Komposition.
    ? [eff.look_mood, eff.color_world, eff.typography_character]
  ...
  const lines = [
    ...(loose ? [
      abstract
        // PFAD B — abstrakte Text-Beschreibung, KEIN Bild angehängt.
        ? "ABSTRACT TEXT-ONLY BRIEF (important — this OVERRIDES every reference and style-matching instruction " +
          "below): There is NO reference image attached to this request — ignore any wording below that says a " +
          "reference / moodboard image 'is provided' or to match it. Work ONLY from the abstract stylistic " +
          "description in this prompt: take only the GENERAL direction (overall mood, genre, rough colour mood " +
          "as a principle, general typographic character). INVENT your own imagery, motif, composition, layout " +
          "and specific colours that fit that general vibe. Do NOT reconstruct or imitate any specific source " +
          "flyer — not its background, decorative graphics, splatters, textures, colour combination, motif or " +
          "composition. Create a fully ORIGINAL, independent club flyer that merely shares the broad vibe."
        // PFAD A — Referenzbild geht mit, aber konkrete Elemente NICHT übernehmen.
