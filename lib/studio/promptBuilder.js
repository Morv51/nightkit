...
// ── Auto-Flow 2: pro Textrolle typografische Attribute + Beispieltext wörtlich in den
//    Prompt (opt-in via opts.typo + dna.text_elements). Wird NUR gesetzt, wenn die
//    erweiterte Analyse gelaufen ist; sonst greift der Standardpfad unverändert. ──
...
// Eine Zeile je Textelement: exakter Beispieltext + seine Attribute ausformuliert.
function typoElementLines(elements) {
  const out = [];
  for (const el of Array.isArray(elements) ? elements : []) {
    if (!el || typeof el !== "object") continue;
    const role = String(el.role || "").toLowerCase();
    const label = ROLE_LABEL_TYPO[role] || (role ? role.toUpperCase() : "TEXT");
    const sample = v(el.sample);
    if (!sample) continue;
    const attrs = [];
    if (v(el.type_class)) attrs.push(v(el.type_class));
    if (v(el.weight)) attrs.push(v(el.weight) + " weight");
    if (v(el.slant)) attrs.push(v(el.slant));
    if (v(el.tracking)) attrs.push(v(el.tracking) + " letter-spacing");
    if (v(el.effect) && v(el.effect).toLowerCase() !== "none") attrs.push(v(el.effect) + " effect");
    const size = parseInt(el.size_pct, 10);
    if (Number.isFinite(size) && size > 0) attrs.push("about " + size + "% of the headline size");
    if (v(el.align)) attrs.push(v(el.align) + "-aligned");
    out.push('- ' + label + ': render the exact text "' + sample + '"' + (attrs.length ? " — " + attrs.join(", ") + "." : "."));
  }
  return out;
}
...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  const lines = [
    ...
    ...(typo ? [
      "TYPOGRAPHIC TREATMENT PER TEXT ELEMENT — this IS the design hierarchy, read from the reference; give " +
        "each element its OWN deliberate treatment as specified below, do NOT flatten everything to one uniform " +
        "style and NEVER render neutral system type (no plain Arial / Helvetica). Legibility ALWAYS comes first; " +
        "keep ONE coherent type system (not five random fonts); the hierarchy must not collapse (secondary texts " +
        "never outshine the headline); type stays straight and flat — no 3D, no perspective, no slanted or " +
        "distorted lettering:",
      ...typoElementLines(d.text_elements),
  ...
  lines.push(
    ...
    "",
    ...(typo ? [
      "TEXT CONTENT: the flyer shows EXACTLY the text elements specified above and NO other text — render each " +
        "element's example text exactly as written, correctly spelled, no gibberish, no extra words, labels, " +
        "headings or taglines. These are EXAMPLE placeholder contents and will be swapped for the user's real " +
        "texts later; use NO real club, label, brand, festival or artist names. Each element appears EXACTLY " +
        "ONCE on the whole flyer — never repeat or duplicate any element, not even inside a badge, stamp or circle.",
    ] : [
