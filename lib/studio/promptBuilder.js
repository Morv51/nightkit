...
// Modus 2: baut den Generierungs-Prompt für einen NEUEN Flyer aus der Stil-DNA.
// Das Moodboard liefert NUR den Look (Stil-Guideline) — KEINE Inhalte/Objekte
// werden übernommen. Reine Text-zu-Bild-Generierung (kein Edit/keine Referenz).
// Das Ergebnis ist im UI editierbar; diese Funktion liefert den Default.
function buildMoodboardPrompt(dna, opts = {}) {
  const d = dna || {};
  const time = v(opts.time) || TIME_PLACEHOLDER;
  const style = [
    v(d.look_mood), v(d.color_world), v(d.imagery_style), v(d.typography_character),
    v(d.composition), v(d.visual_hierarchy), v(d.texture_grain_light), v(d.editorial_club_look),
  ].filter(Boolean).join(" · ") || "modern editorial club aesthetic";

  return [
    "Create a NEW vertical 9:16 club event flyer FROM SCRATCH.",
    "Use ONLY the following as STYLE GUIDELINE (do NOT copy any objects or content from the moodboard, only adopt its look):",
    style + ".",
    "",
    "The flyer MUST contain these placeholder texts, clearly legible, well-set typography, correct hierarchy:",
    `HEADLINE, SUBLINE, ${DATE_PLACEHOLDER}, ${time}, LOCATION, DJ NAME 1, DJ NAME 2, DJ NAME 3, CLUBNAME, ${WEBSITE_PLACEHOLDER}.`,
    "",
    "Single flyer filling the frame, high resolution, print-quality. No logos, no watermarks, no extra text beyond the placeholders. This is an original design inspired by the style, NOT a reproduction of the moodboard.",
  ].join("\n");
}
