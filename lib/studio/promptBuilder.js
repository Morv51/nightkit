...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  // Varianten-Hervorhebung: nur die geänderten Aspekte explizit betonen.
  const ov = [];
  if (v(vr.color_world)) ov.push("color world = " + v(vr.color_world));
  if (v(vr.imagery_style)) ov.push("motifs / illustration = " + v(vr.imagery_style));
  if (v(vr.mood)) ov.push("mood = " + v(vr.mood));
  if (ov.length) {
    lines.push("");
    lines.push("VARIATION (keep the rest of the style identical, change ONLY this): " + ov.join("; ") + ".");
  }
