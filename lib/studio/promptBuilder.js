...
function buildMoodboardPrompt(dna, opts = {}) {
  ...
  const lines = [
    ...
    // Regelwerk (A/B-Schalter, opt-in): fertig ausgewaehlte Zeilen aus designRulesPrompt —
    // universelle + die stilabhaengigen der erkannten Kategorie. Steht NACH der Leitidee (die
    // Referenz bleibt vorrangig) und VOR den technischen Bloecken, die es nur ergaenzt. Ohne
    // Schalter ist die Liste leer und der Prompt bleibt Zeichen fuer Zeichen der bisherige.
    ...(Array.isArray(opts.regelLines) ? opts.regelLines.filter((x) => v(x)) : []),
    ...(loose ? [
...
// Welche Rollen deckt eine geprüfte Zuordnung NICHT ab — in der Schreibweise, die das
// destillierte Regelwerk kennt (designRulesPrompt.ROLLE_LABEL: datum/uhrzeit/location/djs).
// Genauer als dna.fehlende_rollen aus der Vision, weil hier von Hand geprüft wurde. Rollen
// ohne Entsprechung im Regelwerk (HEADLINE, SUBLINE, CLUBNAME, WEBSITE) fallen raus — für
// sie gibt es dort ohnehin keine Herleitung.
function missingRuleRoles(zones) {
  const assigned = new Set();
  for (const z of zones || []) if (z && ROLE_PLACEHOLDERS[z.role] && v(z.text)) assigned.add(z.role);
  const out = [];
  if (!assigned.has("DATUM")) out.push("datum");
  if (!assigned.has("UHRZEIT")) out.push("uhrzeit");
  if (!assigned.has("LOCATION")) out.push("location");
  // "djs" gilt nur als fehlend, wenn der Flyer KEINEN einzigen DJ-Namen zeigt. Zeigt er einen,
  // ist die Setzweise der Rolle sichtbar und das Bildmodell braucht keine Herleitung dafür.
  if (!["DJ NAME 1", "DJ NAME 2", "DJ NAME 3"].some((r) => assigned.has(r))) out.push("djs");
  return out;
}
...
function buildPlaceholderPrompt(zones, opts = {}) {
  ...
  // Regelwerk (A/B-Schalter, opt-in): fertig ausgewaehlte Zeilen aus designRulesPrompt, wie
  // im Auto-Flow. Ohne Schalter ist die Liste leer und der Prompt bleibt Zeichen fuer Zeichen
  // der bisherige — der von Hand kopierte Prompt aendert sich also nicht.
  const regelLines = (Array.isArray(opts.regelLines) ? opts.regelLines : []).filter((x) => v(x));
  const assigned = new Set();
  const replace = [];
  const remove = [];
  const keep = [];
  ...
  // ── Variante (opt-in via opts.variant): DIESELBE Rollen-Zuordnung, dasselbe Regelwerk,
  //    dieselbe Typo-Sorgfalt und derselbe Motiv-Schutz — nur Farbwelt, Motiv und Platzierung
  //    duerfen abweichen. Ohne opts.variant ist isVariant=false und JEDE der fuenf betroffenen
  //    Stellen faellt auf ihren bisherigen Wortlaut zurueck: der Hauptflyer bleibt (bis auf die
  //    immer aktive Platzhalter-Literalsperre) Zeichen fuer Zeichen der bisherige. ──
  const vr = opts.variant || {};
  const isVariant = !!(v(vr.color_world) || v(vr.imagery_style) || v(vr.layout));
  const varLines = [];
  if (v(vr.color_world)) varLines.push("- New colour world: " + v(vr.color_world));
  if (v(vr.imagery_style)) varLines.push("- New imagery motif: " + v(vr.imagery_style));
  if (v(vr.layout)) varLines.push("- New arrangement: " + v(vr.layout));

  // Worauf sich die Platzierung stuetzt (Abschnitt 2 + LAYOUT-Block): in der Variante die neue
  // Anordnung, sonst das destillierte Anordnungsmuster — das steht aber nur im Prompt, wenn das
  // Regelwerk eingespeist wurde. Ohne beides bleibt es bei bewusster Art-Direction, aber NIE bei
  // einer festen Reihenfolge (genau die erzeugte den gequetschten Stapel).
  // BEWUSST ohne "above"/"below": der Regelblock steht UNTER Abschnitt 2, aber UEBER dem
  // LAYOUT-Block — eine Richtungsangabe waere an einer der beiden Stellen schlicht falsch.
  const platzierungRef = isVariant ? "the new arrangement"
    : (regelLines.length ? "the SECONDARY-BLOCK ARRANGEMENT" : "deliberate art direction");

  return [
    // (1/5) INTRO — Variante: Handwerk + Fonts bleiben, Farbwelt/Motiv/Platzierung frei.
    isVariant
      ? "Rebuild this event flyer into a reusable template, as a DISTINCT VARIATION (its new colour " +
        "world, imagery motif and arrangement are specified below). Keep the SAME typographic craft and " +
        "the SAME fonts as the original, but you MAY freely change the colour world, the imagery motif " +
        "and the placement of elements. Never place text over the main visual subjects of the flyer " +
        "(e.g. people/faces, products, vehicles, logos, key graphic elements) — keep the central motif " +
        "fully visible and uncovered. Do not change the fonts."
    ...
    "",
    // Abwechslungs-Teil: die drei geaenderten Achsen explizit, nur in einer Variante.
    ...(isVariant ? [
      "THIS IS A VARIATION (important): produce a DISTINCT variation that deliberately DIFFERS from the " +
        "original in colour world, imagery motif and arrangement, exactly as specified here. It must NOT " +
        "be a clone of the original. Keep it unmistakably the same CRAFT and quality — the same " +
        "typographic care and the same font character — but give it its own look:",
      ...varLines,
      "",
    ] : []),
    ...
    "",
    // (2/5) REPLACE-Kopf — die Replace-ZEILEN selbst bleiben identisch (Rollen-Zuordnung),
    //       nur die Positions-Vorgabe wird in der Variante geloest.
    isVariant
      ? "1) These existing texts become their placeholders — keep each in the SAME typographic style, " +
        "but place them wherever the new arrangement calls for. Once replaced, that field is complete; " +
        "do NOT add it again anywhere else:"
    ...
    "",
    ...(addLines.length ? [
      // Frueher: "gathered into one tidy, well-organised info area (group them, do not scatter)".
      // Das zwang die ergaenzten Rollen in EINEN kompakten Klotz und arbeitete gegen ein Muster,
      // das die Infos bewusst auf mehrere das Motiv umrahmende Anker verteilt. Jetzt: sauber
      // gesetzt und gruppiert, aber nach dem Anordnungsmuster VERTEILT statt gedraengt. Chaotisch
      // verstreut bleibt verboten — "do not scatter" gilt weiter, nur nicht als Block-Zwang.
      "2) Add the MISSING placeholders. Set them deliberately as designed groups and place them " +
        "according to " + platzierungRef + ". They MAY sit as several deliberate anchors distributed " +
        "across the flyer, framing the motif — do NOT cram them together into one compact block just " +
        "because they are missing. Each group stays internally tidy and aligned: nothing is randomly " +
        "scattered, but nothing is forced into a single box either:",
      ...addLines,
    ] : [
      "2) All placeholders already exist — only the replacements above are needed.",
    ]),
    "",
    // Regelwerk direkt NACH den fehlenden Rollen: dort traegt die destillierte Herleitung
    // ("die Rolle fehlt im Original, leite sie so ab") unmittelbar. Die universellen Regeln
    // gelten fuer den ganzen Flyer und stehen damit vor den technischen Bloecken.
    ...(regelLines.length ? [...regelLines, ""] : []),
    ...(remove.length ? [
    ...
    "",
    // PLATZHALTER-LITERALSPERRE (IMMER aktiv, beide Pfade): ein Template ist eine befuellbare
    // Vorlage, kein fertiger Flyer. Verhindert, dass ein Platzhalter durch einen konkreten
    // Beispielwert ersetzt wird — egal ob der aus dem Regelwerk-Muster oder dem Bildmodell kaeme.
    "PLACEHOLDER TEXT IS LITERAL (STRICT — this is a fillable template, NOT a finished flyer):",
    "- Render EVERY placeholder as the exact literal word listed above and nothing else: HEADLINE, " +
      "SUBLINE, the date " + DATE_PLACEHOLDER + ", the time " + TIME_PLACEHOLDER + ", LOCATION, DJ NAME 1, " +
      "DJ NAME 2, DJ NAME 3, CLUBNAME, the website " + WEBSITE_PLACEHOLDER + ".",
    "- NEVER substitute a placeholder with a concrete example value. The TIME stays the word " +
      TIME_PLACEHOLDER + " and must NOT become a clock time such as 22:00, 8 PM or 20 Uhr. The LOCATION " +
      "stays the word LOCATION and must NOT become a real venue name. The DJ slots stay DJ NAME 1 / 2 / 3 " +
      "and must NOT become real artist names. The DATE stays " + DATE_PLACEHOLDER + " and the WEBSITE " +
      "stays " + WEBSITE_PLACEHOLDER + ". Output the placeholder words verbatim — not translated, not " +
      "altered, not replaced by an example.",
    "",
    // (3/5) DESIGN-IDENTITAET — Fonts + Motiv-Schutz bleiben immer; nur die zwei Zeilen, die
    //       Konzept und Farbwelt/Motiv festnageln, weichen in der Variante auf.
    isVariant ? "KEEP — the craft (non-negotiable); everything else may change for this variation:"
      : "KEEP — design identity (non-negotiable):",
    isVariant
      ? "- A NEW creative direction is wanted WITHIN the same craft: you MAY reimagine the concept, the " +
        "colour world and the motif. Do not cling to the original's concept."
    ...
    isVariant
      ? "- Take the colour world, textures and imagery motif in the NEW direction specified above — they " +
        "need NOT match the original; only the typographic craft and the fonts carry over unchanged."
    ...
    "",
    // (4/5) YOU MAY — in der Variante ausdruecklich frei umkomponieren.
    ...(isVariant ? [
      "YOU MAY and SHOULD recompose freely (this is a variation):",
      "- Rearrange, resize and restage the imagery, the text blocks and the graphic elements to realise " +
        "the new arrangement above.",
      "- This variation may look distinctly different from the original in composition, colour and motif " +
        "— that is wanted, as long as the main subject stays whole and clearly recognisable and every " +
        "field fits cleanly without overlapping.",
    ] : [
    ...
    "",
    // (5/5) LAYOUT — EIN Block fuer BEIDE Wege. Frueher stand hier im Ein-Bild-Pfad ein starrer
    //       "LAYOUT (zones, top to bottom)" mit fester Reihenfolge (Headline, Core-Info,
    //       DJ-Stack, Footer). Der widersprach dem SECONDARY-BLOCK ARRANGEMENT weiter oben, und
    //       gpt-image folgte dem konkreteren alten Block: alles gequetscht in eine Ecke
    //       gestapelt. Jetzt folgt die Platzierung in beiden Wegen dem Muster bzw. der
    //       Varianten-Vorgabe, nie einer festen Reihenfolge. Verbot des platten Stapels und
    //       Motiv-Schutz bleiben (Typo-Qualitaet).
    isVariant ? "LAYOUT — art-directed for this variation:" : "LAYOUT — art-directed, never a fixed order:",
    isVariant && v(vr.layout)
      ? "- Arrange the composition as: " + v(vr.layout) + ". Place the DOMINANT headline and group the " +
        "secondary info deliberately (offset / asymmetric / balanced), NEVER a plain left-aligned stacked column."
      : "- Compose deliberately and art-directed: a dominant headline, the secondary info grouped and " +
        "balanced, NEVER a plain left-aligned stacked column.",
    "- HEADLINE / SUBLINE dominant; DATE / TIME / LOCATION as a designed core-info block; DJ NAME 1-3 " +
      "as a designed line-up (never a flat, dead stack); CLUBNAME and WEBSITE small — but their " +
      "PLACEMENT follows " + platzierungRef + ", not a fixed top-to-bottom order.",
