...
function buildPlaceholderPrompt(zones, opts = {}) {
  ...
  // Worauf sich die Platzierung im LAYOUT-Block stuetzt: in der Variante die neue Anordnung,
  // sonst das destillierte Anordnungsmuster — das steht aber nur im Prompt, wenn das Regelwerk
  // eingespeist wurde. Ohne beides bleibt es bei bewusster Art-Direction, aber NIE bei einer
  // festen Reihenfolge (genau die erzeugte den gequetschten Stapel).
  const platzierungRef = isVariant ? "the new arrangement"
    : (regelLines.length ? "the SECONDARY-BLOCK ARRANGEMENT above" : "deliberate art direction");
