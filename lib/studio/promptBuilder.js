...
function buildPlaceholderPrompt(zones, opts = {}) {
  ...
  for (const z of list) {
    if (z && ROLE_PLACEHOLDERS[z.role] && v(z.text)) {
      replace.push(`- "${v(z.text)}" -> "${ROLE_PLACEHOLDERS[z.role]}"`);
      assigned.add(z.role);
    }
  }

  // Build-Modus: NUR Ersetzen, maskiert. Der Prompt beschreibt ausschließlich
  // die Textzonen — alles andere bleibt unangetastet (Maske + Composite sorgen
  // pixelgenau dafür). Keine Ergänzungen, keine Layout-/Hierarchie-Eingriffe.
  if (opts.replaceOnly) {
    return [
      "You are editing ONLY the masked text areas of this flyer (inpainting). Inside the " +
        "masked areas, replace the existing text with these placeholders, keeping each one's " +
        "ORIGINAL font, size, colour, casing, alignment and position:",
      "",
      ...(replace.length ? replace : ["- (none)"]),
      "",
      "Match the original letterforms exactly. Do not add, move, restyle or remove anything; " +
        "render nothing outside the marked areas. Keep the background inside each area intact.",
    ].join("\n");
  }
...
// Hierarchie-Hinweis je Rolle für den einzelnen Add-Schritt.
const ROLE_LEVEL = {
  HEADLINE: "the dominant headline", SUBLINE: "a secondary subline",
  DATUM: "medium-size core info", UHRZEIT: "medium-size core info", LOCATION: "medium-size core info",
  "DJ NAME 1": "a medium-small line-up entry", "DJ NAME 2": "a medium-small line-up entry",
  "DJ NAME 3": "a medium-small line-up entry",
  CLUBNAME: "a small, understated footer line", WEBSITE: "a small, understated footer line",
};

// Add-Schritt (separat, gezielt): einen einzelnen Platzhalter in die maskierte
// Zielbox setzen. opts.infoRef = {text, font} als Stil-Anker für Info-Ebenen.
function buildAddPrompt(role, opts = {}) {
  const placeholder = ROLE_PLACEHOLDERS[role] || role;
  const isHeadline = role === "HEADLINE" || role === "SUBLINE";
  const lines = [
    `Inside the masked area only, add the text "${placeholder}" as ${ROLE_LEVEL[role] || "a text element"}.`,
  ];
  if (isHeadline) {
    lines.push("Use the flyer's distinctive display/headline font and style.");
    const h = refDesc(opts.headlineRef);
    if (h) lines.push(`Follow the style of the existing text ${h}.`);
  } else {
    lines.push("Do NOT use the headline display font — use a clean, legible secondary font that fits the flyer.");
    const i = refDesc(opts.infoRef);
    if (i) lines.push(`Match the style of the existing text ${i}: same font character, casing and kerning.`);
  }
  lines.push("Render it cleanly on the existing background, well-aligned with proper spacing. " +
