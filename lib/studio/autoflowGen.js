...
// Clean-Flow-Varianten: EIN Sonnet-Aufruf aus dem BILD -> N Design-Familie-Vorgaben, jede zu
// einem kurzen Clean-Flow-Varianten-Bildprompt gebaut. Kein Regelwerk, keine Analyse-DNA, keine
// Anordnungsregeln. -> [{ label, prompt }] wie variantPrompts, damit die Kachel-Schleife sie nutzt.
async function cleanVariants({ imageBase64, mediaType, count, model }) {
  if (!vision.isConfigured()) throw new Error("ANTHROPIC_API_KEY not configured");
  const specs = await vision.cleanVariantSpecs({ imageBase64, mediaType, count, model });
  ...
  return specs.map((s) => ({
    ...
    prompt: cleanFlow.buildCleanVariantPrompt({ color_world: s.color_world, imagery_style: s.imagery_style, layout: s.layout }),
  }));
}
