...
async function redesignGate({ original, candidate, fieldList, layoutCard, mediumCard, familyText, model }) {
  const out = await vision.redesignGate({ original, candidate, fieldList, layoutCard, mediumCard, familyText, model });
...
async function redesignFamilyRoll({ images, model, voice }) {
  const { accent, tone, raw } = await vision.redesignFamilySpecs({ images, model });
  try { usage.count("claude_familyspecs"); } catch (_) {}
  // voice (Bauteil E): optionale Schriftstimme aus dem Dialog, serverseitig in den Absatz gebaut.
  const family_text = promptCore.buildFamilyText(accent, tone, voice);
  return { accent, tone, voice: (typeof voice === "string" ? voice.trim() : ""), family_text, raw };
}
...
// model/size: nur der Weg "Analyse-Prompt -> Auto-Flow" setzt sie. Bleiben sie leer,
// gelten wie bisher die Modul-Defaults von openaiImage — bestehende Aufrufer senden
// zeichengleich dieselbe Nutzlast wie zuvor.
//
// rawPrompt: schaltet deescalate AB. Die siebzehn Ersetzungen sind fuer INTERN
// erzeugte Prompts gedacht. Ein von Hand getrimmter Produktionsprompt darf nicht
// stillschweigend umgeschrieben werden, sonst gehen genau die Formulierungen
// verloren, auf die er hin gebaut wurde. Fuer alle bestehenden Wege bleibt
// deescalate unveraendert in Kraft.
async function generateFinalImage({ prompt, imageBuffer, imageType, textOnly, model, size, rawPrompt }) {
  if (!OPENAI_KEY) throw new Error("OPENAI_API_KEY not configured");
  const sentPrompt = rawPrompt ? prompt : deescalate(prompt);
  let image;
  if (textOnly) {
    // Die WIRKSAME Groesse — ohne size ist das der Modul-Default, die Outpaint-Pruefung
    // unten verhaelt sich damit exakt wie vorher.
    const effSize = size || openaiImage.GEN_SIZE;
    const out = await openaiImage.generateImage({
      apiKey: OPENAI_KEY, prompt: sentPrompt, moderation: "low", timeoutMs: 180000,
      ...(model ? { model } : {}), ...(size ? { size } : {}),
    });
    try { usage.count("openai_gptimage"); } catch (_) {}
    image = out.b64 ? `data:image/png;base64,${out.b64}` : await downloadToDataUrl(out.url);
    // Nur der 2:3-Weg braucht den fal-Nachzug. Bei nativem 9:16 faellt er weg.
    if (effSize === "1024x1536") { try { image = await outpaintTo916(image); } catch { /* 2:3 behalten */ } }
  } else {
