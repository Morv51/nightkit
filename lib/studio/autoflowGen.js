...
async function cleanVariants({ imageBase64, mediaType, count, model, subject, artwork }) {
  if (!vision.isConfigured()) throw new Error("ANTHROPIC_API_KEY not configured");
  const { variants } = await vision.cleanVariantSpecs({ imageBase64, mediaType, count, model, artwork: !!artwork });
  try { usage.count("claude_variantspecs"); } catch (_) {}
  return (variants || []).map((s) => ({
    label: s.label || String(s.color_world || "").slice(0, 70),
    prompt: artwork
      ? cleanFlow.buildCleanArtworkPrompt({ color_world: s.color_world })
      : cleanFlow.buildCleanVariantPrompt({ color_world: s.color_world, subject }),
  }));
}
