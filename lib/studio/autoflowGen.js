...
async function analyze({ imageBase64, mediaType, model, loose }) {
  if (!vision.isConfigured()) throw new Error("ANTHROPIC_API_KEY not configured");
  const dna = await vision.analyzeReference({ images: [{ base64: imageBase64, mediaType }], refType: "single", model });
  try { usage.count("claude_analyze"); } catch (_) {}
  const mainPrompt = promptBuilder.buildMoodboardPrompt(dna, { refType: "single", looseInspiration: !!loose });
  return { dna, mainPrompt };
}

// Varianten-Prompts (Sonnet) -> [{ label, prompt }]. Identisch zu /admin/auto-variants.
async function variantPrompts({ dna, count, model, loose }) {
  if (!vision.isConfigured()) throw new Error("ANTHROPIC_API_KEY not configured");
  ...
  return specs.map((s) => ({
    ...
    prompt: promptBuilder.buildMoodboardPrompt(dna, {
      refType: "single", looseInspiration: !!loose,
