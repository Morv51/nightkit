...
async function cleanRedesign({ imageBase64, mediaType, model }) {
  if (!vision.isConfigured()) throw new Error("ANTHROPIC_API_KEY not configured");
  const { spec, raw } = await vision.cleanRedesignSpecs({ imageBase64, mediaType, model });
...
module.exports = { parseDataUrl, deescalate, downloadToDataUrl, outpaintTo916, loadRuleset, analyze, variantPrompts, cleanVariants, cleanArtworks, cleanRedesign, generateFinalImage };
