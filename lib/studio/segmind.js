...
function nanoBody(g) {
  return { prompt: g.prompt, ...(g.imageUrls.length ? { image_urls: g.imageUrls } : {}),
    aspect_ratio: g.aspectRatio, safety_tolerance: Math.min(g.safety, 6), output_format: "png" };
}
...
// 1152x2048 = exakt 9:16 (beide Vielfache von 16). WICHTIG: safety_tolerance > 5
// verlangt bei FLUX eine Sonderautorisierung (HTTP 400 "requires authorization"),
// daher auf max. 5 deckeln = permissivste Stufe ohne Extra-Freigabe.
function fluxBody(g) {
  return { prompt: g.prompt, ...(g.imageUrls.length ? { image_urls: g.imageUrls } : {}),
    width: 1152, height: 2048, safety_tolerance: Math.min(g.safety, 5), output_format: "png" };
}
