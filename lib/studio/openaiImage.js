...
// durchgereicht — nur sein per Vision extrahierter Stil steckt im Prompt. So
// entsteht ein NEUER Flyer im Stil, keine Reproduktion. Klares 90s-Timeout,
// OpenAI-Fehlermeldung wird im Klartext durchgereicht (Auth/Rate-Limit/Moderation/
// Guthaben). Hochformat: gpt-image bietet als höchstes Portrait 1024x1536 (≈2:3,
// nicht exakt 9:16); per Env OPENAI_IMAGE_SIZE überschreibbar.
const GEN_PATH = "/v1/images/generations";
const GEN_TIMEOUT_MS = 90 * 1000;
const GEN_SIZE = (process.env.OPENAI_IMAGE_SIZE && process.env.OPENAI_IMAGE_SIZE !== "auto")
  ? process.env.OPENAI_IMAGE_SIZE
  : "1024x1536";

function generateImage({ apiKey, prompt, size = GEN_SIZE, quality = "high" }) {
  return new Promise((resolve, reject) => {
    if (!apiKey) return reject(new Error("OPENAI_API_KEY not configured"));
    if (!prompt) return reject(new Error("Prompt missing"));
    const payload = JSON.stringify({ model: MODEL, prompt, size, quality, n: 1 });
    ...
    req.setTimeout(GEN_TIMEOUT_MS, () => req.destroy(new Error("OpenAI-Zeitüberschreitung nach 90 s")));
    req.on("error", reject);
    req.write(payload);
    req.end();
  });
...
module.exports = { editImage, generateImage, MODEL };
