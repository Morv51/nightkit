...
function editImage({ apiKey, prompt, imageBuffer, imageType = "image/png", size = SIZE, quality = "high", inputFidelity = INPUT_FIDELITY }) {
...
function generateImage({ apiKey, prompt, size = GEN_SIZE, quality = "high" }) {
  return new Promise((resolve, reject) => {
    ...
    killer = setTimeout(() => done(() => {
      req.destroy();
      reject(new Error("Zeitüberschreitung: OpenAI hat nicht innerhalb von 90 s geantwortet"));
    }), GEN_TIMEOUT_MS);
