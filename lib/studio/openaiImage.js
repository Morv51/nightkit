...
// Eigener Satz fuer den Weg "Analyse-Prompt -> Auto-Flow": dort wird 9:16 NATIV
// erzeugt. gpt-image-2 nimmt beliebige WIDTHxHEIGHT an, solange beide Kanten
// durch 16 teilbar sind und das Verhaeltnis zwischen 1:3 und 3:1 liegt.
// 1152x2048 ist exakt 9:16 (1152/16=72, 2048/16=128) und bleibt mit 2,36 Mio.
// Pixeln unter der Experimentell-Schwelle von 2560x1440. Weil die Groesse damit
// NICHT "1024x1536" ist, faellt der fal-Outpaint in autoflowGen von selbst weg.
// Beides per Env umstellbar, ohne Deploy.
const PROMPT_GEN_MODEL = process.env.OPENAI_IMAGE_GEN_MODEL || "gpt-image-2";
const PROMPT_GEN_SIZE = process.env.OPENAI_IMAGE_GEN_SIZE || "1152x2048";

function generateImage({ apiKey, prompt, size = GEN_SIZE, model = MODEL, quality = "high", moderation, timeoutMs = GEN_TIMEOUT_MS }) {
  return new Promise((resolve, reject) => {
    if (!apiKey) return reject(new Error("OPENAI_API_KEY not configured"));
    if (!prompt) return reject(new Error("Prompt missing"));
    ...
    const genPayload = { model, prompt, size, quality, n: 1 };
...
module.exports = { editImage, generateImage, MODEL, GEN_SIZE, PROMPT_GEN_MODEL, PROMPT_GEN_SIZE };
