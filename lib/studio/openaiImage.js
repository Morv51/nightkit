...
// MOTIVERHALT (Kern-Fix): gpt-image edit verwirft OHNE input_fidelity=high das
// Originalmotiv und malt Personen neu (Default ist niedrig). "high" bewahrt
// Gesichter/Personen/Details des Eingabebildes — genau das macht auch die
// ChatGPT-Oberfläche bei einem solchen Edit. Per Env abschaltbar/änderbar.
const INPUT_FIDELITY = process.env.OPENAI_INPUT_FIDELITY || "high";

// Harte Motiverhalt-Regel, die JEDEM GPT-Edit ganz vorangestellt wird.
const MOTIF_PRESERVE =
  "This is an EDIT of the provided image. Keep the existing people/subjects EXACTLY as they are — " +
  "same persons, same number, same poses, same faces, same clothing. Do NOT replace, swap, restyle " +
  "or re-generate the people or the main subject; preserve the original photo/artwork and its details. " +
  "Only change the TEXT (replace and add the placeholders exactly as described below).";
...
function editImage({ apiKey, prompt, imageBuffer, imageType = "image/png", size = SIZE, quality = "high" }) {
  return new Promise((resolve, reject) => {
    ...
    // Motiverhalt-Regel ganz nach vorne; input_fidelity=high bewahrt das Eingabebild.
    const fields = { model: MODEL, prompt: MOTIF_PRESERVE + "\n\n" + prompt, size, quality, n: "1" };
