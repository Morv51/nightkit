    ...
    for (const file of state.files) {
      ...
      if (!file.mainPrompt) {
        try {
          ...
          const a = await gen.analyze({ imageBase64: input.buffer.toString("base64"), mediaType: input.contentType, model: state.model, loose: state.loose, typo });
          file.dna = a.dna; file.mainPrompt = a.mainPrompt;
          if (state.variants > 0) {
            try { file.variantPrompts = await gen.variantPrompts({ dna: a.dna, count: state.variants, model: state.model, loose: state.loose, typo }); }
