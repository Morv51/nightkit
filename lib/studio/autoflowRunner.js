...
const promptCore = require("../../prompts/core"); // Redesign v2: Feldliste + Karten fuers Gate
...
// Bauteil A.3: Fehlertyp bestimmen. limit VOR safety (ein Guthaben-/Billing-Fehler darf nie als
// Motiv-Reject gelten). safety = das Bildmodell hat das MOTIV abgelehnt (loest KEIN Nachwuerfeln
// aus, Bauteil A.4 — dasselbe Motiv wuerde wieder geblockt). Sonst ein gewoehnlicher Fehler.
function classifyError(msg) {
  const s = String(msg || "");
  if (/insufficient|quota|billing|credit|rate.?limit|payment|exceeded your current|too many requests|429/i.test(s)) return "limit";
  if (/safety|moderation|content.?polic|rejected|\[sexual\]|not allowed|violat|safety_violation/i.test(s)) return "safety";
  return "error";
}
...
async function startRun(spec) {
  ...
  // ZWEITER EINGANG: fertige Kacheln, die ihren Prompt schon mitbringen. Dann braucht
  // der Lauf KEIN Eingangsbild — es gibt nichts zu analysieren, der Prompt steht bereits.
  // Genutzt vom Reiter "Analyse-Prompt". Ohne spec.tiles bleibt alles wie bisher.
  const readyTiles = Array.isArray(spec.tiles)
    ? spec.tiles.filter((t) => t && typeof t.prompt === "string" && t.prompt.trim())
    : [];
  if (!files.length && !readyTiles.length) throw new Error("Keine Dateien");
  ...
  if (readyTiles.length) {
    // Prompt-Lauf: EINE Pseudo-Datei, damit die Orchestrierung ihre Schleife ueber
    // state.files faehrt. mainPrompt ist gesetzt, damit der Analyse-Block uebersprungen
    // wird. Kein putInput, kein srcExt — getInput wird nie gerufen, weil textOnly gilt.
    stateFiles.push({
      fnum: 1, name: String(spec.sourceName || "Analyse-Prompt"), srcExt: "", dna: null, srcTemplate: "",
      mainPrompt: readyTiles[0].prompt, variantPrompts: [], analyzeError: null,
      handZones: null, infoRef: null,
    });
    for (let k = 0; k < readyTiles.length; k++) {
      tiles.push({
        fnum: 1, index: "1-v" + (k + 1), kind: "variant", num: k + 1,
        status: "pending", reason: "", prompt: readyTiles[k].prompt,
      });
    }
  } else {
  ...
  const state = {
    ...
    loose: !!spec.loose, textOnly: !!spec.textOnly, variants: readyTiles.length || variants,
    model: spec.model || "sonnet", refType: spec.refType || "single",
    // Bild-Modell und -Groesse NUR fuer den Prompt-Lauf. Leer -> die Modul-Defaults von
    // openaiImage, also zeichengleich wie bisher fuer alle bestehenden Flows.
    imgModel: String(spec.imgModel || ""), imgSize: String(spec.imgSize || ""),
    // rawPrompt: deescalate abschalten. Nur der Prompt-Lauf setzt das.
    ...
    // Redesign v2: die AUFTRAGS-Familie (Bauteil 3). Alle Templates dieses Auftrags teilen sie.
    // Leer -> familienloser Modus, der [FAMILIE]-Slot im Kern faellt weg.
    cleanFamilyText: String(spec.cleanFamilyText || ""),
...
// Redesign v2 (core): der Bildprompt je Vorlage aus Kern + Karten-Kombination (rank =
// templateIndex = fnum-1) + Auftrags-Familie. Stempelt die Karten auf Datei UND v1-Kachel,
// damit Lauf-Karte und Gate sie kennen. Reine Assemblierung, kein Sonnet-Aufruf.
function redesignCorePromptsFor(state, file) {
  const rank = (Number(file.fnum) || 1) - 1;
  const built = gen.redesignCorePrompt({ familyText: state.cleanFamilyText, rank });
  file.cards = { layoutKey: built.layoutKey, mediumKey: built.mediumKey, rank: built.rank };
  file.familyText = state.cleanFamilyText || "";
  const tile = state.tiles.find((t) => t.fnum === file.fnum && t.num === 1);
  if (tile) tile.cards = file.cards;
  return [{ label: built.layoutKey + " · " + built.mediumKey, prompt: built.prompt }];
}

// Das Qualitaets-Gate fuer EINEN frisch erzeugten Kandidaten (Bauteil 4). Nur core-Redesign.
// Schreibt tile.gate = { verdict, reasons }. Wirft nie — ein Fehler wird zu verdict "error".
async function runRedesignGate(state, file, tile, original, candidate) {
  if (!state.cleanRedesign || REDESIGN_PROMPT_MODE !== "core") return;
  try {
    const cards = tile.cards || file.cards || {};
    const g = await gen.redesignGate({
      original: { base64: original.buffer.toString("base64"), mediaType: original.contentType },
      candidate: { base64: candidate.buffer.toString("base64"), mediaType: candidate.mediaType },
      fieldList: promptCore.placeholderList(promptCore.FULL_REDESIGN_FIELDS),
      layoutCard: require("../../prompts/cards").cardText("layout", cards.layoutKey),
      mediumCard: require("../../prompts/cards").cardText("medium", cards.mediumKey),
      familyText: state.cleanFamilyText || "", // Bauteil C: Familien-Treue (Punkt 12) nur mit Familie
      model: state.model,
    });
    tile.gate = { verdict: g.verdict, reasons: g.reasons };
    log("Gate " + tile.index + ": " + g.verdict + (g.reasons.length ? " (" + g.reasons.join("; ") + ")" : ""));
  } catch (e) {
    tile.gate = { verdict: "error", reasons: [e && e.message ? e.message : String(e)] };
  }
}

// Redesign v2, Nachwuerfeln (Bauteil 4): liess das Gate nach einem Auftrag mit >=3 Templates
// weniger als 2 Kandidaten durch, bekommen bis zu 2 DURCHGEFALLENE Templates je einen frischen
// Kandidaten mit der naechsten unbenutzten Karten-Kombination (rank ab N). Kostenschutz: hoechstens
// 2 Nachwuerfe je Auftrag. Idempotent: existieren schon Nachwurf-Kacheln, wird nicht erneut
// gewuerfelt (ein fortgesetzter Lauf erzeugt sie ueber die Hauptschleife zu Ende).
async function maybeRerollRedesign(runId, state) {
  if (!state.cleanRedesign || REDESIGN_PROMPT_MODE !== "core") return;
  if (cancelledNow(runId, state)) return;
  if (state.tiles.some((t) => t.reroll)) return; // schon gewuerfelt
  const N = state.files.length;
  if (N < 3) return;
  const passCount = state.tiles.filter((t) => t.gate && t.gate.verdict === "pass").length;
  if (passCount >= 2) return;
  const failed = state.tiles.filter((t) => t.kind === "variant" && !t.reroll && t.gate && t.gate.verdict === "fail").slice(0, 2);
  if (!failed.length) return;

  let rank = N; // naechste unbenutzte Kombination in der teilerfremden Folge
  for (let k = 0; k < failed.length; k++) {
    if (cancelledNow(runId, state)) return;
    const src = failed[k];
    const file = state.files.find((f) => f.fnum === src.fnum);
    if (!file) continue;
    const built = gen.redesignCorePrompt({ familyText: state.cleanFamilyText, rank: rank++ });
    const tile = {
      fnum: file.fnum, index: file.fnum + "-r" + (k + 1), kind: "variant", num: 100 + k,
      status: "pending", reason: "", reroll: true,
      cards: { layoutKey: built.layoutKey, mediumKey: built.mediumKey, rank: built.rank },
      prompt: built.prompt,
    };
    state.tiles.push(tile);
    await persist(state);
    log("Nachwurf " + tile.index + " (" + tile.cards.layoutKey + " · " + tile.cards.mediumKey + ")");
    tile.status = "running"; await persist(state);
    try {
      const input = await store.getInput(runId, file.fnum, file.srcExt);
      const image = await gen.generateFinalImage({ prompt: built.prompt, textOnly: false, imageBuffer: input.buffer, imageType: input.contentType });
      const outPic = gen.parseDataUrl(image);
      if (!outPic) throw new Error("Kein Ergebnisbild");
      await store.putResultImage(runId, tile.index, outPic.buffer);
      tile.status = "done"; await persist(state);
      await runRedesignGate(state, file, tile, input, outPic);
      await persist(state);
    } catch (e) {
      tile.status = isBlocked(e.message) ? "blocked" : "error"; tile.reason = e.message; tile.errKind = classifyError(e.message);
      await persist(state);
    }
  }
}
  ...
  try {
    ...
    for (const file of state.files) {
        ...
        if (state.cleanFlow) {
          if (state.variants > 0) {
            try {
              ...
              // Redesign v2 (core): KEIN Sonnet-Regie-Aufruf, der Bildprompt entsteht rein aus
              // Kern + Karten-Kombination (rank = templateIndex) + Auftrags-Familie. 'legacy'
              // faehrt weiter ueber die Sonnet-Regie mit Richtungs-Rotation.
              file.variantPrompts = state.cleanRedesign
                ? (REDESIGN_PROMPT_MODE === "core"
                    ? redesignCorePromptsFor(state, file)
      ...
      for (const t of fileTiles) {
        ...
        // t.prompt: von Nachwurf-Kacheln direkt gesetzt (Bauteil 4); sonst der uebliche Weg.
        const prompt = t.prompt || (t.kind === "main" ? file.mainPrompt : (file.variantPrompts[t.num - 1] && file.variantPrompts[t.num - 1].prompt));
        ...
        try {
          ...
          const image = await gen.generateFinalImage({
            ...
            model: state.imgModel, size: state.imgSize, rawPrompt: !!state.rawPrompt,
          });
          const outPic = gen.parseDataUrl(image);
          if (!outPic) throw new Error("Kein Ergebnisbild");
          await store.putResultImage(runId, t.index, outPic.buffer);
          t.status = "done"; await persist(state);
          // Qualitaets-Gate direkt nach dem frischen Bild (nur core-Redesign; input ist geladen).
          await runRedesignGate(state, file, t, input, outPic);
          await persist(state);
        } catch (e) {
          t.status = isBlocked(e.message) ? "blocked" : "error"; t.reason = e.message; t.errKind = classifyError(e.message);
          await persist(state);
        }
      }
    }
    // Redesign v2: Nachwuerfeln, falls das Gate zu wenige Kandidaten durchgelassen hat.
    await maybeRerollRedesign(runId, state);
...
module.exports = { startRun, cancelRun, resumeRun, recoverRunning, orchestrate, classifyError };
