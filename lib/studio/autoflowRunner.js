...
// Hintergrund starten. spec: { flow, mode, loose, textOnly, variants, model, refType, files:[{name,dataUrl}] }
async function startRun(spec) {
  ...
  for (let i = 0; i < files.length; i++) {
    const fnum = i + 1;
    const pic = gen.parseDataUrl(files[i].dataUrl);
    if (!pic) throw new Error("Ungueltiges Bild (Datei " + fnum + ")");
    const { ext } = await store.putInput(runId, fnum, pic.buffer, pic.mediaType);
    stateFiles.push({ fnum, name: String(files[i].name || ("flyer" + fnum)), srcExt: ext, dna: null, mainPrompt: "", variantPrompts: [], analyzeError: null });
    tiles.push({ fnum, index: fnum + "-haupt", kind: "main", num: 0, status: "pending", reason: "" });
    ...
    for (const file of state.files) {
      ...
      if (!file.mainPrompt) {
        try {
          ...
          const a = await gen.analyze({ imageBase64: input.buffer.toString("base64"), mediaType: input.contentType, model: state.model, loose: state.loose, typo, ruleset });
          file.dna = a.dna; file.mainPrompt = a.mainPrompt;
          if (state.variants > 0) {
            try { file.variantPrompts = await gen.variantPrompts({ dna: a.dna, count: state.variants, model: state.model, loose: state.loose, typo, ruleset }); }
