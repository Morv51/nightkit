...
Return a JSON array of exactly n flat objects. All objects share identical family, staging, headline_style and placements. Only palette and label differ, so the set reads as one design family.

{
  "label": "name of the colour direction, max 6 words",
  "family": "subject" or "surface",
  "staging": "how the main element enters the frame and which edges it runs past, max 20 words. For surface: how the photo fills the frame and where the calm areas are",
  "headline_style": "the character of the headline typeface, max 12 words",
...
const ARTWORK_FIELDS = ["label", "family", "staging", "headline_style",
  "place_headline", "place_date", "place_lineup", "place_club", "palette", "style"];
const ARTWORK_SHARED = ["family", "staging", "headline_style",
...
function anchorArtworkRegie(specs) {
  ...
  const regie = { family };
  for (const k of ARTWORK_SHARED) {
    if (k === "family") continue;
    regie[k] = (fits.find((s) => s[k]) || {})[k] || "";
  }
  return specs.map((s) => ({ ...s, ...regie }));
}
...
async function cleanArtworkSpecs({ imageBase64, mediaType, count, model }) {
  ...
  async function attempt() {
    const message = await client.messages.create({
      model: modelId(model),
      max_tokens: 6000, // zehn Felder je Eintrag statt einer Farbwelt
    ...
    const specs = arr.slice(0, n).map((x) => {
      const o = {};
  ...
  return { specs: anchorArtworkRegie(out.specs), raw: out.raw };
}

// ── REDESIGN-Regie (NUR Redesign-Modus, eigener Aufruf). Eingabe ist ein FERTIGES Template:
//    Foto, Komposition und Farbwelt bleiben, nur die Textsetzung wird neu gemacht. Darum KEIN
//    staging (die Komposition steht), KEINE palette (die Farbwelt bleibt), KEIN family.
//    Genau EIN Objekt je Vorlage — jede Vorlage bekommt ihren eigenen Aufruf, also gibt es hier
//    auch nichts zu verankern. Zusaetzlich zum Artwork-Muster ("nicht beschreiben, nur Orte")
//    die Auflage, dass jede Position in einer FREIEN Flaeche liegt und das Motiv nicht verdeckt. ──
const REDESIGN_DIRECTION_SYSTEM = `You are the art director for a club poster that already exists. You see the poster.

Do not describe the photograph. Do not name what is in it, do not use adjectives about it, do not invent elements. You only decide where the text goes.

The photograph, the composition and the colour world stay exactly as they are. Only the typography is set again.
...

{
  ...
  "carriers": "which groups sit on a carrier and what the carrier is, max 12 words"
}

Rules for your decisions:
- Every position must lie in a FREE area of the existing composition. Never place text over the main subject, and never over busy photographic detail.
- The four groups sit far apart, each in its own area. Never two of them stacked in the same column.
- A carrier is a small physical object a single line sits on: torn paper, tape, a tag, a label, a solid bar. At most two of the four groups get one, and only single lines.
- Carriers come from the world of the photograph. A raw analog photo takes torn paper and tape. A glossy photo takes solid bars. A soft luminous photo takes no carrier at all.
...
const REDESIGN_FIELDS = ["headline_style", "place_headline", "place_date", "place_lineup", "place_club", "carriers"];

// -> { spec:{ …6 Felder… } | null, raw }. KEIN Pflichtfeld: der Eintrag gilt erst als leer, wenn
// ALLE sechs Felder leer sind (dann greift beim Aufrufer der bestehende Fehlerpfad). raw ist der
// Rohtext — nur fuer die Vorschau, NICHT fuer den Lauf-Zustand.
async function cleanRedesignSpecs({ imageBase64, mediaType, model }) {
  if (!client) throw new Error("ANTHROPIC_API_KEY not configured");
  const s = (x) => (typeof x === "string" ? x.trim() : "");
  ...
  async function attempt() {
    const message = await client.messages.create({
      model: modelId(model),
      max_tokens: 2000, // ein Objekt mit sechs kurzen Feldern
...
module.exports = { isConfigured, analyzeMoodboard, analyzeReference, generateVariationSpecs, cleanVariantSpecs, cleanArtworkSpecs, cleanRedesignSpecs, variantPrompts, analyzeFlyer, tagFlyer, analyzeDesignPatterns, distillDesignRules, MODELS };
