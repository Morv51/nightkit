...
// Erweiterter Analyse-Tail NUR fuer Auto-Flow 2 (opt-in via typo=true). Zusaetzlich zu den
// 8 Stil-Keys: pro Textrolle typografische Attribute + ein kurzer, markenrechtssicherer
// Beispieltext, plus die Varianz der Nebentexte. Alle anderen Flows nutzen weiter
// STYLE_KEYS_TAIL (unveraendert).
const TYPO_KEYS_TAIL =
  ...
  "('left' | 'center' | 'right'). Judge every attribute FAITHFULLY from the reference. " +
...
async function analyzeReference({ images, refType, model, typo }) {
  const table = typo ? REF_SYSTEM_TYPO : REF_SYSTEM;
  ...
  // sonst schneidet das JSON ab.
  return visionCallMulti({ system: sys, instruction, images, model, maxTokens: typo ? 4000 : 2000 });
}
