...
// Artwork-Modus: Sonnet SIEHT das Foto und waehlt ACCENT-Farbwelten, die zum Grundton des Fotos
// KONTRASTIEREN (sonst geht der Text im Foto unter). Ueber die Varianten deutlich verschieden, nie
// mehrfach dieselbe Familie. Rot moeglich, aber NICHT Voreinstellung; bei S/W- oder entsaettigten
// Fotos ebenso Cyan/Giftgruen/Magenta/Gelb/Orange. Nach Kontrast und Wirkung, nicht nach Gewohnheit.
const CLEAN_ARTWORK_SYSTEM =
  "You are an art director. You receive ONE PHOTO from which a club event flyer will be built. Design a set of " +
  "ACCENT colour worlds, one per variation, to sit against this photo. " +
  "Each accent must CONTRAST with the photo's dominant tone and stand out clearly, never blend in or get lost " +
  "in it. Across the set the accents must be DISTINCTLY different from one another — not the same family twice. " +
  "Give ONLY a colour world per variation, a specific colour combination. Describe NOTHING else. " +
  "Red is one possible choice but NOT the default; for a black-and-white or desaturated photo, cyan, acid " +
  "green, magenta, yellow or orange are just as valid. Choose by contrast and impact, not by habit. " +
  "Reply with STRICT JSON only (no markdown): { \"variants\": [ { \"label\": \"…\", \"color_world\": \"…\" } ] }.";
...
// artwork=true -> Foto-Kontrast-Anweisung statt Flyer-Kollektion.
async function cleanVariantSpecs({ imageBase64, mediaType, count, model, artwork }) {
  if (!client) throw new Error("ANTHROPIC_API_KEY not configured");
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      system: artwork ? CLEAN_ARTWORK_SYSTEM : CLEAN_VARIANT_SYSTEM,
      messages: [{
        role: "user",
        content: [
          { type: "text", text: artwork
            ? ("Here is the photo. Design exactly " + n + " accent colour worlds that each CONTRAST with the " +
               "photo and are distinctly different from one another. Reply with STRICT JSON only, no prose.")
            : ("Here is the original flyer. Design exactly " + n + " matching colour worlds as one family, " +
