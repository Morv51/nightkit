...

{
  ...
  "headline_style": "the single most important design decision. Choose a lettering direction with real character that carries the poster's world, and describe its letterforms concretely, max 15 words. The direction varies from poster to poster: an energetic brush script, a high-contrast fashion serif, an ultra-condensed poster grotesque, a rounded liquid display face, a sharp angular techno face, drawn letterforms where one letter carries a pictorial element from the scene. Never a plain neutral sans, never the same safe choice twice",
  "headline_material": "the hard material from the picture the headline letters are cut from, e.g. the marble of the columns. Empty only if none is visible, max 12 words. Use it only when the material genuinely elevates the headline beyond what the letterforms achieve. Most posters need none. Name the material itself, no leading verbs such as cut from or made of",
...
- Look for a hard, nameable material prominent in the picture: stone, marble, metal, glass, wood, paper. If one exists, use it. Leave the field empty only when no such material is visible.
