...
const DESIGN_RULES_SYSTEM =
  ...
  "Write all texts in GERMAN, concise and concrete. Reply with STRICT JSON only (no markdown, no commentary) " +
...
async function distillDesignRules({ records, model, maxTokens = 8000 }) {
  ...
  const out = await distillCall({
    ...
    userText: "Diese " + n + " Datensaetze stammen aus " + n + " echten, professionell gestalteten Flyern. Finde " +
      "die WIEDERKEHRENDEN Anordnungsmuster des Sekundaer-Blocks ueber ALLE hinweg und forme die typische " +
      "Grafiker-Anordnung (ein anordnung_muster-Absatz, konkrete regeln, herleitung_rollen fuer fehlende Rollen). " +
      "Nenne fuer jede Regel und jede Herleitung die belege (idx-Werte). Antworte mit striktem JSON.\n\n" +
