...
const VARIANT_PROMPTS_SYSTEM =
  ...
  "STRICTLY PRESERVE and keep IDENTICAL across all variants: the overall VIBE and mood, the genre and energy, " +
  "the general imagery language and illustration / rendering style, and the ROUGH COMPOSITION and layout " +
  "structure of the original. Do NOT scatter or randomise layout or genre. " +
  "VARY ONLY, deliberately: (1) the COLOUR WORLD — a different colour combination per variant, still fitting " +
  "the same kind of flyer and the same overall intensity family (a dark flyer stays dark, a light one stays " +
  "light); (2) DETAIL ELEMENTS — smaller decorative details, textures, accents, secondary motifs and finishing " +
  "touches. Keep the main subject, the composition and the mood constant; change only colours and small details. " +
  "Each PROMPT must be COMPACT (roughly 60-120 words) yet COMPLETE and self-contained, directly usable in an " +
  "image generator: faithfully describe the flyer's style, mood, composition and typographic character, then " +
  "state THIS variant's specific colour world and detail changes. " +
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          { type: "text", text:
            `Analyse this flyer and produce EXACTLY ${n} variant prompts. Keep the vibe, genre and composition ` +
            `identical; vary ONLY the colour world and the detail elements. Reply with STRICT JSON only, no prose.` },
          { type: "image", source: { type: "base64", media_type: mediaType, data: imageBase64 } },
        ],
      }],
    });
