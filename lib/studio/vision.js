...
//    Varianten DESSELBEN Flyers (nur Text, keine Bilderzeugung). Vibe strikt erhalten
//    (Stimmung, Genre, Bildsprache, Grundkomposition); variiert werden NUR Farbwelt +
//    Detailelemente. EIN Sonnet-Aufruf; Robustheit wie bei den Varianten-Specs. ──
const VARIANT_PROMPTS_SYSTEM =
  ...
  "of the original. Every one of the variants must read as a polished, ready-to-sell CLUB / event flyer, NOT as " +
  "fine art, a horror poster or a chaotic art piece. Concretely, each prompt must actively call for: a clear " +
  "typographic hierarchy with ONE dominant headline plus a subline; a cleanly set, well-organised info block " +
  "carrying the placeholder date, DJ names, location, clubname and website; generous, calm negative space rather " +
  "than an overfilled surface; clearly legible, well-set text; and a balanced, deliberately art-directed " +
  "composition. Word each prompt for professional-grade production quality: professional graphic design, clean " +
  "typography, strong readability, balanced layout, agency-level polish. " +
  "VARY, but ONLY to a MODERATE, controlled degree (this is the only real variable): the central MOTIF, the " +
  "image IDEA and the concrete COMPOSITION may differ from variant to variant — not the same design ten times — " +
  "but ALWAYS inside the professional club-flyer structure above and WITHOUT sacrificing quality. Prefer " +
  "moderate, controlled, consistently high-quality variation over wild divergence; genre, occasion and mood of " +
  "the original stay the same throughout. Keep it GENERAL for any flyer genre: never name specific example " +
  "motifs — describe the carrying elements of THIS uploaded flyer abstractly. " +
  "PRIORITY on any conflict: QUALITY and genuine club-flyer character come FIRST, variation breadth SECOND. If " +
  "they clash, make a variant somewhat more similar but keep it high-quality and club-ready, rather than " +
  "original but cheap-, chaotic- or overloaded-looking. " +
  "Each PROMPT must be COMPACT (roughly 70-130 words) yet COMPLETE and self-contained, directly usable in an " +
  "image generator: state the genre, the preserved colour palette / colour mood and the overall vibe faithfully " +
  "to the reference; explicitly require the professional club-flyer structure and quality (dominant headline, " +
  "subline, clean info block, calm negative space, legible text, balanced professional layout, 9:16); then " +
  "describe THIS variant's own motif, image idea and composition WITHIN that structure. " +
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          { type: "text", text:
            `Analyse this flyer and produce EXACTLY ${n} variant prompts. Keep the genre, mood level, 9:16 format ` +
            `and ESPECIALLY the colour palette almost identical to the original, and keep EVERY variant a polished, ` +
            `high-quality CLUB flyer: clear typographic hierarchy (dominant headline + subline), a clean info block, ` +
            `calm negative space, readable text, balanced professional layout. Vary only MODERATELY the central ` +
            `motif, image idea and composition — controlled, always high-quality, never a horror poster or chaotic ` +
            `art. Quality and club-flyer character come BEFORE variation breadth. Reply with STRICT JSON only, no prose.` },
          { type: "image", source: { type: "base64", media_type: mediaType, data: imageBase64 } },
        ],
      }],
    });
