...
// ── Clean-Flow (eigenstaendig): EIN Bild-Aufruf -> ein vibe_anchor (einmal pro Lauf) + N kurze
//    Varianten-Vorgaben als Design-FAMILIE. Bekommt den Original-Flyer als Bild (nicht die
//    DNA-Analyse). Sonnet gibt KEINE Anordnung mehr vor (das liess das Bildmodell einen Bauplan
//    abarbeiten statt zu komponieren); die Anordnung wird im Bildprompt fest freigegeben. Je
//    Variante nur noch color_world + ein KURZES, vages imagery_style. Der vibe_anchor benennt die
//    tragenden Bauteile des Originals konkret und steht in JEDER Variante. KEINE Anordnungsregeln,
//    kein Zonen-Zwang — das ist NICHT das Regelwerk. ──
const CLEAN_VARIANT_SYSTEM =
  "You are an art director designing a small COLLECTION of variations of ONE club event flyer. You receive the " +
  "original flyer as an image. " +
  "FIRST, in ONE 'vibe_anchor' shared by the WHOLE collection, name the CARRYING elements of the original " +
  "CONCRETELY — the structural pieces that make it recognisable, e.g. \"a strong single portrait as the hero, a " +
  "large flowing graphic letterform behind the figure, scattered petal accents, a dominant blocky headline " +
  "lockup, a big stacked date on one side\". Every variation keeps this anchor. " +
  "THEN design the variations as ONE cohesive collection with a clear red thread, all sharing that anchor and the " +
  "same mood, energy and typographic treatment. Across the collection vary ONLY two things: " +
  "color_world — a specific colour combination, e.g. \"deep crimson and charcoal with silver accents\"; " +
  "imagery_style — SHORT and vague: only the TYPE and MOOD of the person/motif, e.g. \"a different stylish " +
  "figure, same confident street vibe\". Do NOT describe clothing, hair, gaze or other small details — they " +
  "distract the image model from the composition. If the flyer shows a person, vary the person (including " +
  "gender); if it shows no person, vary the motif. " +
  "Do NOT prescribe any arrangement or layout — the flyer is recomposed freely elsewhere. " +
  "PROGRESSION: variation 1 stays CLOSE to the original in colour world and look — a light, fresh take, NOT a " +
  "clone. Each further variation drifts progressively further while staying unmistakably in the family. " +
  "Reply with STRICT JSON only (no markdown): { \"vibe_anchor\": \"…\", \"variants\": [ { \"label\": \"…\", " +
  "\"color_world\": \"…\", \"imagery_style\": \"…\" } ] }.";

// -> { vibe_anchor, variants:[{ label, color_world, imagery_style }] }. Robustheit wie
// generateVariationSpecs: tolerante Feldnamen, Rettung einzelner Objekte bei kaputtem Gesamt-JSON,
// ein Wiederholversuch bei leerem Ergebnis (dann geht der vibe_anchor beim Salvage-Fall verloren
// und der Bildprompt faellt auf das generische "keep the original's vibe" zurueck).
async function cleanVariantSpecs({ imageBase64, mediaType, count, model }) {
  ...
  async function attempt() {
    const message = await client.messages.create({
      model: modelId(model),
      max_tokens: 4000, // reichlich Kopffreiheit fuer bis zu 10 kurze Varianten
      system: CLEAN_VARIANT_SYSTEM,
      messages: [{
        role: "user",
        content: [
          { type: "text", text: "Here is the original flyer. First give the shared vibe_anchor, then design " +
            "exactly " + n + " variations as one design family, variation 1 close to the original. Reply with " +
    ...
    try {
      ...
      else if (parsed && typeof parsed === "object") { obj = parsed; arr = Array.isArray(parsed.variants) ? parsed.variants : []; }
    } catch { arr = salvageObjects(text); }
    const variants = arr.slice(0, n).map((x) => ({
      label: pick(x, "label"),
      color_world: pick(x, "color_world", "colorWorld", "colors", "color", "palette"),
      imagery_style: pick(x, "imagery_style", "imageryStyle", "imagery", "subject", "motif", "person"),
    })).filter((x) => x.color_world || x.imagery_style);
    return { vibe_anchor: pick(obj || {}, "vibe_anchor", "vibeAnchor", "anchor", "carrying_elements"), variants };
  }
