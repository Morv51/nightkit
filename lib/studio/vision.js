...
async function distillCall({ system, userText, model, maxTokens, abschnitt }) {
  ...
  if (message && message.stop_reason === "max_tokens") {
    throw new Error(abschnitt + ": Antwort an der Token-Grenze abgeschnitten (max_tokens=" + maxTokens + ", len=" + raw.length + ")");
  }
  try { return parseJSON(raw); }
  catch (e) { throw new Error(abschnitt + ": JSON nicht lesbar [len=" + raw.length + "]"); }
}
