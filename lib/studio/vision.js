...
const REDESIGN_GATE_SYSTEM = (fieldList, layoutCard, mediumCard) =>
`Du prüfst einen automatisch erzeugten Club-Poster-Kandidaten (Bild 2) gegen eine feste Qualitäts-Checkliste. Bild 1 ist die Original-Vorlage, aus der er entstand. Du bist streng: im Zweifel durchgefallen.
...
11 POSTER-NIVEAU: Das Ganze wirkt wie die Arbeit eines Designers, Bild und Text bilden ein Stück. Wirkt es wie Text auf ein Foto gelegt: durchgefallen.
...
Antworte NUR mit diesem JSON:
{
  "checks": {
    ...
    "11_poster_niveau": true/false
  },
  "verdict": "pass" | "fail",
  "reasons": ["kurze Begründung je gescheitertem Punkt"]
}
verdict ist nur dann "pass", wenn alle elf Punkte bestehen.`;

async function redesignGate({ original, candidate, fieldList, layoutCard, mediumCard, model }) {
  if (!client) throw new Error("ANTHROPIC_API_KEY not configured");
  const message = await client.messages.create({
    model: modelId(model), max_tokens: 700,
    system: REDESIGN_GATE_SYSTEM(fieldList || "", layoutCard || "", mediumCard || ""),
    messages: [{
      role: "user",
      content: [
  ...
  try {
    ...
    const allPass = Object.keys(checks).length === 11 && Object.values(checks).every((x) => x === true);
    // "pass" nur, wenn Sonnet pass sagt UND alle elf true sind (Doppel-Absicherung).
    ...
    return { verdict, checks, reasons, raw };
  } catch (e) {
    return { verdict: "error", checks: {}, reasons: ["Gate-Antwort nicht lesbar"], raw };
  }
}
...
module.exports = { isConfigured, analyzeMoodboard, analyzeReference, generateVariationSpecs, cleanVariantSpecs, cleanArtworkSpecs, cleanRedesignSpecs, redesignFamilySpecs, redesignGate, variantPrompts, analyzeFlyer, tagFlyer, analyzeDesignPatterns, distillDesignRules, MODELS, REDESIGN_DIRECTIONS };
