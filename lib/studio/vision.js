...
// ── Clean-Flow (eigenstaendig): EIN Bild-Aufruf -> N kurze Varianten-Vorgaben als
//    Design-FAMILIE. Bekommt den Original-Flyer als Bild (nicht die DNA-Analyse) und erfindet je
//    Variante nur drei konkrete Werte: color_world, imagery_style (Person/Motiv), layout. KEINE
//    Anordnungsregeln, kein Zonen-Zwang — das ist NICHT das Regelwerk. Die gemeinsame Handschrift
//    und "erste nah am Original" stecken in DIESER Anweisung, nicht im Bildprompt. ──
const CLEAN_VARIANT_SYSTEM =
  ...
  "original flyer as an image. Design the variations as ONE cohesive collection with a clear red thread: they " +
  "share the SAME handwriting — the same mood and energy, the same typographic treatment, and the same way of " +
  "handling the graphic elements. Across the collection you vary ONLY three things: the colour world, the " +
  "person/motif, and the arrangement. They must read as a family, never as unrelated flyers. " +
  "PROGRESSION: variation 1 stays CLOSE to the original in colour world and overall look — a light, fresh take, " +
  "NOT an exact clone. From there each further variation drifts progressively further, each in its own direction " +
  "(different colour world, different person/motif, bolder arrangement), while staying unmistakably in the same " +
  "family. " +
  "For EACH variation give three SHORT, concrete specs: " +
  "color_world — a specific colour combination, e.g. \"deep crimson and charcoal with silver accents\"; " +
  "imagery_style — a new person or motif in the same vibe, briefly (if the flyer shows a person, vary the person " +
  "including gender and look; if it shows no person, vary the motif); " +
  "layout — a short, art-directed arrangement (just the idea, NO rules, NO zone lists). " +
  "Keep every spec short. Reply with STRICT JSON only (no markdown): { \"variants\": [ { \"label\": \"…\", " +
  "\"color_world\": \"…\", \"imagery_style\": \"…\", \"layout\": \"…\" } ] }.";

// N Varianten-Vorgaben aus dem BILD. Robustheit wie generateVariationSpecs: tolerante Feldnamen,
// Rettung einzelner Objekte bei kaputtem Gesamt-JSON, ein Wiederholversuch bei leerem Ergebnis.
async function cleanVariantSpecs({ imageBase64, mediaType, count, model }) {
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      messages: [{
        role: "user",
        content: [
          { type: "text", text: "Here is the original flyer. Design exactly " + n + " variations as one design " +
            "family, variation 1 close to the original. Reply with STRICT JSON only, no prose." },
          { type: "image", source: { type: "base64", media_type: mediaType, data: imageBase64 } },
        ],
      }],
    });
  ...
  let specs = await attempt();
  if (!specs.length) specs = await attempt();
  return specs;
}
