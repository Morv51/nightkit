...
//    EIN Text-Aufruf (kein Bild) mit ALLEN Datensaetzen gebuendelt. Das Modell extrahiert die
//    WIEDERKEHRENDEN Muster (nicht die Einzelfaelle) und fasst sie in Regeln. Ergebnis in zwei
//    Gruppen: universelle Regeln (ueber (fast) alle Stil-Kategorien) + stilabhaengige Regeln
//    (nach stil_kategorie, mit Fokus herleitung_rollen). Fuer JEDE Regel nennt das Modell die
//    belege = Liste der Datensatz-Indizes; die ANZAHL zaehlen WIR im Code (ehrliche
...
const DESIGN_RULES_SYSTEM =
  "You are a senior graphic designer DISTILLING many individual flyer analyses into ONE compact, reusable " +
  "RULESET. You receive a JSON array of datasets; each has an integer `idx` and the fixed catalogue fields " +
  "(blickfang, flaechenaufteilung, klassen, fehlende_rollen, datum, schrift_disziplin, tiefe, ordnung, vibe, " +
  "stil_kategorie, lautstaerke, herleitung_rollen). " +
  "Your task: extract the RECURRING design PATTERNS across the datasets and phrase them as RULES. Report ONLY " +
  "what REPEATS across MULTIPLE datasets — never restate an individual case, never invent a pattern that is not " +
  "actually in the data. " +
  "Split the result into TWO groups: " +
  "(1) universelle_regeln — patterns that hold across ALL or NEARLY ALL stil_kategorie values. Include a rule " +
  "ONLY if it genuinely appears in the MAJORITY of the datasets (e.g. the split into loud design roles vs. quiet " +
  "info roles, a limited number of type families, the motif overlapping text). " +
  "(2) stilabhaengige_regeln — grouped BY stil_kategorie. Per category, its TYPICAL treatment, and ESPECIALLY " +
  "the herleitung_rollen pattern: how the secondary roles a template often lacks (above all the DJs, also " +
  "location, uhrzeit, datum) are DERIVED in that style — the bezugspunkt (what the design borrows from) plus the " +
  "reduktion (what is taken away or added). " +
  "For EVERY rule you MUST include `belege`: the array of `idx` values of the datasets that ACTUALLY support " +
  "that rule. Be honest and precise — add an idx ONLY if that dataset really shows the pattern, do NOT pad the " +
  "list. The number of belege is how the reader judges how strong a rule is, so it must be truthful. " +
  "Write all RULE TEXTS in GERMAN, concise and concrete. Only include stil_kategorie keys that actually occur " +
  "in the data. " +
  "Reply with STRICT JSON only (no markdown, no commentary) with EXACTLY this shape: " +
  '{ "universelle_regeln": [ { "regel": "…", "belege": [0,3,7] } ], ' +
  '"stilabhaengige_regeln": { "<stil_kategorie>": { ' +
  '"regeln": [ { "regel": "…", "belege": [1,4] } ], ' +
  '"herleitung_rollen": [ { "rolle": "djs", "muster": "bezugspunkt + reduktion in einem Satz", "belege": [1,4] } ] } } }.';

async function distillDesignRules({ records, model, maxTokens = 16000 }) {
  if (!client) throw new Error("ANTHROPIC_API_KEY not configured");
  ...
  // Deterministische Basiszahlen (Grundwahrheit fuer die Belastbarkeit je Kategorie).
  const norm = (v) => String(v == null ? "" : v).trim().toLowerCase();
  const kategorien = {};
  for (const b of bundle) { const k = norm(b.stil_kategorie) || "unbekannt"; kategorien[k] = (kategorien[k] || 0) + 1; }
  ...
  const message = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: [{ type: "text", text:
        "Destilliere aus diesen " + n + " Datensaetzen das Regelwerk. NUR wiederkehrende Muster ueber mehrere " +
        "Datensaetze, keine Einzelfaelle. Nenne fuer JEDE Regel die belege (idx-Liste). Antworte mit striktem JSON.\n\n" +
  ...
  try {
    ...
    // Meist abgeschnittene Antwort (max_tokens erreicht). Klarer Fehler statt generisch,
    // damit der Job-Status die Ursache nennt.
    const cut = message && message.stop_reason === "max_tokens";
    throw new Error("Regelwerk-JSON nicht lesbar" + (cut ? " (Antwort abgeschnitten – zu viele Datensaetze/Regeln)" : "") + " [len=" + raw.length + "]");
  }
  ...
  const uni = (Array.isArray(parsed.universelle_regeln) ? parsed.universelle_regeln : [])
    .map(withCount).sort((a, b) => b.beruht_auf - a.beruht_auf);

  const stilIn = (parsed.stilabhaengige_regeln && typeof parsed.stilabhaengige_regeln === "object") ? parsed.stilabhaengige_regeln : {};
  const stil = {};
  for (const key of Object.keys(stilIn)) {
    const grp = stilIn[key] || {};
    stil[key] = {
      anzahl: kategorien[norm(key)] || 0,
      regeln: (Array.isArray(grp.regeln) ? grp.regeln : []).map(withCount).sort((a, b) => b.beruht_auf - a.beruht_auf),
      herleitung_rollen: (Array.isArray(grp.herleitung_rollen) ? grp.herleitung_rollen : []).map(withCount),
    };
  }
