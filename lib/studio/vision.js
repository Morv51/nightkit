...
const VARIANT_PROMPTS_SYSTEM =
  ...
  "VARY WIDELY and deliberately across the variants (these ARE the variables): the COMPOSITION and pictorial " +
  "build-up, the arrangement and placement of the elements, the TYPOGRAPHY placement, and the GRAPHIC / MOTIF " +
  "elements and the overall image IDEA. Each variant must be an INDEPENDENT image idea — different motifs, a " +
  "different composition, a genuinely different visual concept — while staying in the SAME genre and the SAME " +
  "colour world as the original. Do NOT produce the same layout ten times with different decoration; produce " +
  "clearly DIFFERENT image ideas that a viewer recognises as the same event / genre and the same colours, yet " +
  "distinctly different designs. Keep it GENERAL for any flyer genre: never name specific example motifs — " +
  "describe the carrying elements of THIS uploaded flyer abstractly. " +
  "Each PROMPT must be COMPACT (roughly 60-120 words) yet COMPLETE and self-contained, directly usable in an " +
  "image generator: state the genre, the preserved colour palette / colour mood and the overall vibe faithfully " +
  "to the reference, then describe THIS variant's own composition, layout, typography placement and motif / " +
  "graphic idea. " +
  ...
  async function attempt() {
    const message = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          { type: "text", text:
            `Analyse this flyer and produce EXACTLY ${n} variant prompts. Keep the genre, the overall mood level, ` +
            `the 9:16 format and ESPECIALLY the colour palette / colour world almost identical to the original; ` +
            `vary WIDELY the composition, layout, typography placement and graphic / motif elements so each is a ` +
            `distinct image idea in the same genre and colours. Reply with STRICT JSON only, no prose.` },
          { type: "image", source: { type: "base64", media_type: mediaType, data: imageBase64 } },
        ],
      }],
    });
