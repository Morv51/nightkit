...
const TAG_SYSTEM =
  "You label ONE club / party flyer with a few SHORT search KEYWORDS in GERMAN for a filterable library. " +
  "Cover only what is clearly visible: the MOTIF / subject (e.g. basketball, sport, tanz, auto, blumen, " +
  "totenkopf, palme), the 1–3 DOMINANT colours (e.g. rot, schwarzweiß, pink, gold, blau, lila, grün), the " +
  "MOOD / STYLE (e.g. grunge, elegant, streetwear, retro, neon, minimalistisch, luxus, vintage, y2k), and the " +
  "depicted PERSON only if one is clearly shown (mann, frau). 4–8 keywords total, each ONE word or at most two " +
  "words, ALL lowercase, German, consistent and reusable, no duplicates, no punctuation, no sentences. Reply " +
  "with STRICT JSON only: { \"keywords\": [\"…\", \"…\"] }.";
...
async function tagFlyer({ imageBase64, mediaType, model }) {
  ...
  const arr = Array.isArray(out) ? out : (out && Array.isArray(out.keywords) ? out.keywords : []);
  return arr.map((x) => String(x == null ? "" : x)).filter(Boolean); // Normalisierung macht keywords.js
}

module.exports = { isConfigured, analyzeMoodboard, analyzeReference, generateVariationSpecs, analyzeFlyer, tagFlyer, MODELS };
