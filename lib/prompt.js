...
function buildPrompt(rawEvent) {
  ...
  if (dateStr) {
    lines.push(
      `Replace the date placeholder with: "${dateStr}". Render it in the SAME font, weight, style and colour as the original date text already in this template so it matches the flyer's typography — do NOT fall back to a plain or generic font.\n`
    );
  }
