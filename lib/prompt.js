...
// Die Vorbereitung aus buildPrompt, herausgezogen und exportiert, damit ein
// zweiter Befuellweg DIESELBE Slot-Liste bekommt statt einer Kopie. buildPrompt
// ruft sie unveraendert auf; seine Ausgabe bleibt zeichengleich.
function slotsForEvent(rawEvent) {
  const ev = rawEvent || {};
  ...
  return slotsFor({ headline, subline, dateValue, time, djs, club, location, website });
}

function buildPrompt(rawEvent) {
  const slots = slotsForEvent(rawEvent);
...
module.exports = { buildPrompt, slotsForEvent };
