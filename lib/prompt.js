...
// V4-Prompt (für remix-v4): gibt die Textinhalte klar strukturiert vor —
// Label-Liste mit exakten Werten plus Regeln. Bewusst eigenständig von
// buildPrompt (V3), damit beide Pfade unabhängig justierbar bleiben.
// remix-v4 kennt keine strukturierten JSON-Layout-Parameter (json_prompt gibt
// es laut Doku nur bei generate-v4, ohne Basisbild) — daher ein klar
// gegliederter Text-Prompt.
function buildPromptV4(rawEvent) {
  const ev = rawEvent || {};
  const headline = clean(ev.name).toUpperCase();
  const subline  = clean(ev.prefix);
  const weekday  = clean(ev.day);
  ...
  const dateVal = dq
    ? `weekday "${weekday}", day ${dq.day}, month ${dq.monthNum} ("${dq.monthName}"), year ${dq.year} — ` +
      "mirror the template's own date format, abbreviation style and line stacking exactly"
    : (weekday || dateRaw ? `"${[weekday, dateRaw].filter(Boolean).join(" ")}"` : "");

  const v = (s) => (s ? `"${s}"` : "(none)");

  return [
    "Edit this event flyer template. Keep the visual design, colours, graphics, " +
      "fonts, decorations and layout EXACTLY as they are. Replace ONLY the " +
      "placeholder texts with the values below, then remove every placeholder " +
      "that has no value.",
    "",
    "TEXT CONTENT TO PLACE (use each value EXACTLY as written, character for character):",
    `HEADLINE: ${v(headline)}`,
    `SUBLINE: ${v(subline)}`,
    `DATE: ${dateVal || "(none)"}`,
    `TIME: ${time ? `"${time}" — add as "EINLASS ${time} UHR" only if the template has room near the date, in the template's style` : "(none)"}`,
    `LINEUP, in this order: ${djs.length ? djs.map((d) => `"${d}"`).join(", ") : "(none)"}`,
    `WEBSITE: ${v(website)}`,
    `CLUB NAME: ${v(club)}`,
    `LOCATION: ${v(location)}`,
    "",
    "RULES:",
    "Place each value at the template position that matches its label.",
    "Any field marked (none) has no value: remove that placeholder text entirely " +
      "and fill the area with clean matching background.",
    "Always remove the 'CLUB LOGO' placeholder completely and put no text or " +
      "graphics in that area.",
    "Lineup names must appear exactly as written: never add, keep or invent a " +
      "'DJ' or 'DJane' prefix that is not part of a provided name, never invent " +
      "extra names, and completely remove unused lineup slots.",
    "The final image must never show leftover placeholder words like HEADLINE, " +
      "SUBLINE, CLUB LOGO, LOCATION, CLUB NAME, UHRZEIT or template example names.",
    "Do not invent any text. Render every letter exactly as written.",
  ].join("\n");
}

module.exports = { buildPrompt, buildPromptV4 };
