...
function buildPrompt(rawEvent) {
  ...
  if (ev.hasLogo) {
    ...
    lines.push(`1. Top-centre area (currently "CLUB LOGO") — replace with the host / act names, each on its OWN line, do NOT use commas: ${hosts.map((h) => `"${h}"`).join(", ")}\n`);
  } else if (hosts.length === 1) {
    lines.push(`1. Top-centre area (currently "CLUB LOGO") — replace with: "${hosts[0]}"\n`);
  } else {
    lines.push(`1. Top-centre "CLUB LOGO" area: remove the text and leave it empty.\n`);
  }
