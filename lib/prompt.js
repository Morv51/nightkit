...
function buildPrompt(rawEvent) {
  ...
  lines.push(
    djs.length
      ? `The lineup names, in order: ${djs.map((d) => `"${d}"`).join(", ")}. ` +
        "For the lineup, use each provided artist name exactly as written. " +
        "If a name already includes a prefix like 'DJ' or 'DJane', keep that prefix. " +
        "If a name does NOT include such a prefix, do not add 'DJ' or any other prefix to it. " +
        "Replace the template's lineup placeholders with the provided names in order. " +
        "If there are fewer names than placeholder slots, leave the remaining slots empty " +
        "and do not invent any names. Do not add role labels that were not provided.\n"
  ...
  lines.push(
    club
      ? `Replace the "CLUB NAME" placeholder with: "${club}"\n`
      : 'Remove the "CLUB NAME" placeholder text completely and leave that area empty.\n'
  );

  lines.push(
    location
      ? `Replace the "LOCATION" placeholder with: "${location}"\n`
      : 'Remove the "LOCATION" placeholder text completely and leave that area empty.\n'
  );
