...
function buildPrompt(rawEvent) {
  ...
  if (ev.hasLogo) {
    lines.push(`1. Top-centre area that currently reads "CLUB LOGO": leave it COMPLETELY EMPTY — remove the words "CLUB LOGO" and put no text or graphics there. A logo image will be placed into this empty space afterwards.\n`);
  } else {
    lines.push(`1. Top-centre "CLUB LOGO" area: remove the words "CLUB LOGO" and leave the area empty.\n`);
  }
  ...
  if (hosts.length) {
    const lineup = hosts.join("  ·  ");
    lines.push(`6. DJ / artist line-up — add ONE clean line directly below the pink banner, centred, in small white UPPERCASE letters that match the flyer's typography (clearly smaller than the headline) and stay readable on the background: "${lineup}"\n`);
  }

  if (website) {
    lines.push(`7. Website at the very bottom (ALL CAPS): "${website}"\n`);
  }
