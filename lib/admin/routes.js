...
const PAGE = path.join(__dirname, "..", "..", "public", "admin", "manage.html");
...
function register(router) {
  // ── Seite (oeffentlich; enthaelt nur die Gate-UI, keine Secrets). Nur wenn an. ──
  router.get("/admin/manage", async (req, res) => {
    if (!toolsOn()) { sendError(res, 404, "Nicht gefunden"); return true; }
    try {
      ...
      sendError(res, 404, "Verwaltung nicht gefunden");
    }
    return true;
  });
