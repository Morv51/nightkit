...
const { samplePlan } = require("../prompt");     // Ersetzungsplan fuer die Beispieltext-Vorschau
...
const sampleMap = require("./sampleMap");         // Beispieltext-Tabellen (Beta-Befuellpfad)
...
const vision = require("../studio/vision");      // bestehende Verschlagwortung (tagFlyer) + Grafik-Analyse
const templateSource = require("../templateSource"); // nur nutzen: primeKeywords()
const designAnalysis = require("./designAnalysis");  // Grafik-Analyse: Datensatz-Ablage (R2)
const designRules = require("./designRules");        // Grafik-Analyse Phase 3: Regelwerk-Ablage (R2)
const jobs = require("../jobs");                      // Hintergrund-Jobs (langer Destillier-Lauf)

const UPLOAD_LIMIT = 25 * 1024 * 1024; // Base64-Bild pro Upload-Request

// Aufbewahrung des Destillier-Jobs. Deutlich ueber der Standardfrist von 5 Minuten, weil der
// Sonnet-Lauf ueber ~40 Datensaetze laenger dauern kann: sonst raeumt der Sweeper den noch
// laufenden Job weg und das Polling meldet 404, obwohl das Regelwerk in R2 landet.
...
// Kern des Uploads: aus einem QUELL-Bildpuffer (jpg/png/webp) ein neues, aktives
// Template machen — technischer Name -> Original (JPEG q92) + Thumbnail (WebP 360)
// nach R2 -> Discovery-Registry -> Anzeigename -> optional Tags (Sonnet). Wird von
// /admin/upload UND der Auto-Flow-Uebernahme genutzt, damit ein uebernommenes
// Ergebnis sich exakt wie ein normal hochgeladenes Template verhaelt. Wirft bei
// Eingabefehlern Error mit .status (400/500); der Aufrufer bildet das auf sendError ab.
// Optionaler name -> eigener Anzeigename; sonst der Standard "<Kategorie> <n>".
async function uploadTemplate({ category, imageBuffer, autoTag = true, name: customName }) {
  const cat = typeof category === "string" ? category.replace(/\s+/g, " ").trim() : "";
  if (!cat) throw Object.assign(new Error("Kategorie fehlt"), { status: 400 });
  if (cat.startsWith("_") || cat.includes("/") || cat.includes("\\") || cat.length > 60)
    throw Object.assign(new Error("Ungueltige Kategorie"), { status: 400 });
  if (!imageBuffer || !imageBuffer.length) throw Object.assign(new Error("Bild fehlt oder ungueltig"), { status: 400 });
  ...
  for (let i = 0; i < 8 && !rel; i++) {
    const tech = slugify(cat) + "-" + ymd() + "-" + crypto.randomBytes(3).toString("hex") + ".jpg";
    const cand = cat + "/" + tech;
    if (!(await uploads.keyExists("templates/" + cand))) { rel = cand; origKey = "templates/" + cand; }
  }
  if (!rel) throw Object.assign(new Error("Konnte keinen eindeutigen Namen finden"), { status: 500 });
  ...
  const original = await sharp(imageBuffer).rotate().flatten({ background: "#ffffff" }).jpeg({ quality: 92 }).toBuffer();
  ...
  // 4. Anzeigename (custom oder Standard, vor dem Registrieren berechnet) + Registry.
  const name = (typeof customName === "string" && customName.trim()) ? customName.trim().slice(0, 60) : nextName(cat);
  ...
  return {
    ok: true, file: rel, name, category: cat,
...
function register(router) {
  ...
  router.get("/admin/manage/list", async (req, res) => {
    ...
    const built = templates.listRaw().map((t) => {
      ...
      return {
        ...
        // Beta: fuehrt dieses Template eine Beispieltext-Tabelle? (Markierung + Formular)
        sampleMap: sampleMap.mapFor(t.file),
      };
    });
  ...
  //    Die eigentliche Arbeit macht uploadTemplate() (unten exportiert), damit die
  //    Auto-Flow-Uebernahme EXAKT denselben Weg nutzt. ──
  router.post("/admin/upload", async (req, res) => {
    ...
    try {
      const out = await uploadTemplate({ category, imageBuffer: src, autoTag: b.autoTag !== false });
      sendJson(res, 200, out);
    } catch (e) {
      sendError(res, e.status || 502, e.status ? e.message : "Upload fehlgeschlagen: " + (e && e.message ? e.message : e));
    }
    return true;
  });

  // ── Grafik-Analyse (Phase 1+2): EIN hochgeladener Flyer -> fester Fragenkatalog (Sonnet)
  //    -> strukturierter Datensatz, gesammelt in R2 fuer die spaetere Auswertung (Phase 3).
  //    Das BILD wird NUR analysiert: es wird NICHT als Vorlage gespeichert und nicht
  //    nachgebaut. Der Client ruft dies pro Flyer nacheinander auf. ──
  router.post("/admin/design-analysis/analyze", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res, UPLOAD_LIMIT); if (!b) return true;
    if (!vision.isConfigured()) return sendError(res, 500, "ANTHROPIC_API_KEY nicht gesetzt");
    const src = parseImage(b.image);
    if (!src) return sendError(res, 400, "Bild fehlt oder ungueltig (jpg/jpeg/png/webp, max 15 MB)");
    const model = b.model === "haiku" ? "haiku" : "sonnet";
    try {
      // Fuer die Analyse verkleinern: schneller, guenstiger und sicher unter dem Bild-Limit.
      const small = await sharp(src).rotate()
        .resize({ width: 1600, height: 1600, fit: "inside", withoutEnlargement: true })
        .jpeg({ quality: 85 }).toBuffer();
      const fields = await vision.analyzeDesignPatterns({ imageBase64: small.toString("base64"), mediaType: "image/jpeg", model });
      try { usage.count("claude_analyze"); } catch (_) {} // Nutzungs-Zähler, nachrangig
      const record = await designAnalysis.saveRecord({ sourceName: b.name, model, fields });
      sendJson(res, 200, { ok: true, record });
    } catch (e) {
      sendError(res, 502, "Analyse fehlgeschlagen: " + (e && e.message ? e.message : e));
    }
    return true;
  });

  // ── Alle gesammelten Datensaetze (Grundlage fuer Phase 3). ──
  router.post("/admin/design-analysis/list", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    try { sendJson(res, 200, await designAnalysis.listRecords()); }
    catch (e) { sendError(res, 502, "Konnte nicht laden: " + (e && e.message ? e.message : e)); }
    return true;
  });

  // ── Alle Datensaetze verwerfen (neu anfangen). ──
  router.post("/admin/design-analysis/clear", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    try { sendJson(res, 200, await designAnalysis.clearRecords()); }
    catch (e) { sendError(res, 502, "Loeschen fehlgeschlagen: " + (e && e.message ? e.message : e)); }
    return true;
  });

  // ── Phase 3: Alle Datensaetze zu einem REGELWERK destillieren — EINSTUFIG und UNIVERSELL (EIN
  //    Anthropic-Aufruf ueber ALLE Flyer gemeinsam), damit das gemeinsame Anordnungsmuster des
  //    Sekundaer-Blocks entsteht statt kategorie-getrennter Haeufchen. Extrahiert nur WIEDER-
  //    KEHRENDE Anordnungsmuster (anordnung_muster + konkrete Regeln + Rollen-Herleitung), je
  //    Regel mit Belastbarkeit (beruht_auf). Ergebnis wird als getrennte Datei gespeichert
  //    (designRules), NICHT in den Bildprompt-Code geschrieben.
  //    Laeuft als HINTERGRUND-JOB (der Sonnet-Lauf ueber ~40 Datensaetze ist zu lang fuer ein
  //    synchrones Request-Fenster / Plattform-Timeout): gibt sofort 202 { jobId } zurueck, das
  //    Frontend pollt /distill-status. Das Speichern passiert serverseitig auch dann, wenn der
  //    Browser die Verbindung verliert — der Reload zeigt das Regelwerk dann per /ruleset. ──
  router.post("/admin/design-analysis/distill", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    if (!vision.isConfigured()) return sendError(res, 500, "ANTHROPIC_API_KEY nicht gesetzt");
    ...
    const listed = await designAnalysis.listRecords();
    if (listed.ok === false) return sendError(res, 502, "Datensaetze nicht ladbar: " + (listed.error || "unbekannt"));
    const records = listed.records || [];
    if (!records.length) return sendError(res, 400, "Keine Datensaetze zum Destillieren");

    const jobId = jobs.create(DISTILL_TTL_MS);
    jobs.startSweeper();
    sendJson(res, 202, { jobId, datensaetze: records.length });

    const t0 = Date.now();
    (async () => {
      const ruleset = await vision.distillDesignRules({ records, model });
      const saved = await designRules.saveRuleset({ model, ...ruleset });
      try { usage.count("claude_distill"); } catch (_) {} // Nutzungs-Zähler, nachrangig
      console.log(`[design-analysis] Regelwerk destilliert in ${Date.now() - t0} ms (job ${jobId}, ${records.length} Datensaetze)`);
      jobs.set(jobId, { status: "done", ruleset: saved });
    })().catch((e) => {
      console.error("[design-analysis] Destillieren fehlgeschlagen:", e && e.message ? e.message : e);
      jobs.set(jobId, { status: "error", error: "Destillieren fehlgeschlagen: " + (e && e.message ? e.message : e) });
    });
    return true;
  });

  // ── Phase 3: Status-Polling des Destillier-Jobs. done -> { ruleset }, error -> { error }. ──
  router.post("/admin/design-analysis/distill-status", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    const job = jobs.get(b.jobId);
    if (!job) return sendError(res, 404, "Auftrag nicht gefunden (evtl. abgelaufen) — Reload zeigt ein bereits gespeichertes Regelwerk");
    sendJson(res, 200, job);
    if (job.status !== "pending") jobs.remove(b.jobId);
    return true;
  });

  // ── Phase 3: das aktuell gespeicherte Regelwerk laden (zur Anzeige). ──
  router.post("/admin/design-analysis/ruleset", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    try { sendJson(res, 200, await designRules.loadRuleset()); }
    catch (e) { sendError(res, 502, "Konnte nicht laden: " + (e && e.message ? e.message : e)); }
    return true;
  });

  // ── Phase 3: das gespeicherte Regelwerk verwerfen. ──
  router.post("/admin/design-analysis/ruleset-clear", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    try { sendJson(res, 200, await designRules.clearRuleset()); }
    catch (e) { sendError(res, 502, "Loeschen fehlgeschlagen: " + (e && e.message ? e.message : e)); }
    return true;
  });
  ...
  // ── BETA "Beispieltext-Templates" (Bauteil 3 + 4) ──
  // Tabelle je Template speichern/loeschen. Leere Tabelle -> Eintrag faellt weg und das
  // Template laeuft wieder den unveraenderten Platzhalter-Pfad.
  router.post("/admin/manage/sample-map", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    const file = fileOf(b);
    if (!isKnown(file)) return sendError(res, 400, "Unbekanntes Template");
    try {
      const saved = await sampleMap.setMap(file, b.map);
      sendJson(res, 200, { ok: true, file, map: saved, hasSample: !!saved });
    } catch (e) { sendError(res, 502, "Konnte Beispieltext-Tabelle nicht speichern: " + e.message); }
    return true;
  });

  // Ersetzungsplan (Vorschau, kostenlos): zeigt VOR einem bezahlten Lauf, welcher Beispieltext
  // durch welche Eingabe ersetzt und welcher entfernt wird. Leitet aus DENSELBEN slots ab wie
  // der Prompt-Bau, kann also nicht vom spaeteren Lauf abweichen.
  router.post("/admin/manage/sample-plan", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    const file = fileOf(b);
    if (!isKnown(file)) return sendError(res, 400, "Unbekanntes Template");
    // Ungespeicherte Tabelle aus dem Formular hat Vorrang -> Plan schon vor dem Speichern.
    const map = (b.map && typeof b.map === "object") ? b.map : sampleMap.mapFor(file);
    const plan = samplePlan(b.ev || {}, map);
    if (!plan) return sendError(res, 400, "Keine Beispieltext-Tabelle fuer dieses Template");
    sendJson(res, 200, { ok: true, file, ...plan });
    return true;
  });
  ...
  // ── STAPEL-Umformung: ALLE aktiven 2:3-nah-Templates per teurem fal-Modell auf 9:16
  //    bringen und direkt aktiv schalten; die 2:3-Originale wandern in den Papierkorb
  //    (weich, wiederherstellbar). Additive Admin-Funktion, aendert keinen Kern-Flow und
  //    nicht lib/fal.js. Der Lauf laeuft im HINTERGRUND (Klick-Anfrage antwortet sofort),
  //    das Frontend pollt mode "status". mode "preview" liefert nur Anzahl + Kostenschaetzung. ──
  router.post("/admin/reformat-916", async (req, res) => {
    if (!toolsOn()) return sendError(res, 404, "Nicht gefunden");
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    const mod = require("./reformat916");
    try {
      if (b.mode === "status") { sendJson(res, 200, mod.batchStatus()); return true; }
      if (b.mode === "preview") { sendJson(res, 200, mod.previewCount()); return true; }
      const j = mod.startBatch();
      sendJson(res, 200, { status: j.status, total: j.total, done: j.done });
    } catch (e) { sendError(res, 502, "Umformung fehlgeschlagen: " + (e && e.message ? e.message : e)); }
    return true;
  });
...
module.exports = { register, uploadTemplate };
