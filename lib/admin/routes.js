...
function register(router) {
  ...
  router.post("/admin/test-outpaint-916", async (req, res) => {
    ...
    if (!ok(req, res)) return true;
    const b = await body(req, res); if (!b) return true;
    const mod = require("../../test-outpaint-916");
    try {
      // mode "status": aktuellen Job-Stand liefern (schnell). mode "find": nur Plan, direkt.
      // sonst: Hintergrund-Job starten und SOFORT antworten (kein Warten auf fal, kein Haengen).
      if (b.mode === "status") { sendJson(res, 200, mod.jobStatus()); return true; }
      if (b.mode === "find") { sendJson(res, 200, await mod.run("find")); return true; }
      const j = mod.startJob();
      sendJson(res, 200, { status: j.status, startedAt: j.startedAt });
