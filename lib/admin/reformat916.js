...
// den Auto-Flows, der Generierung oder der Lese-Schicht (nutzt sie nur). lib/fal.js
// (Normalbetrieb: image-apps-v2) bleibt unberuehrt; hier wird ausdruecklich das teure
// fal-ai/flux-2-pro/outpaint verwendet, direkt ueber @fal-ai/client.
//
...
const MODEL = "fal-ai/flux-2-pro/outpaint"; // BEWUSST das teure Modell, NUR fuer diese Umformung
const TARGET = 0.5625;                        // 9:16
const COST_PER_MP = 0.03;                     // grobe Kostenschaetzung flux-2-pro (USD je Megapixel)
...
function previewCount() {
  ...
  return { total: targets.length, estCost: Math.round(targets.length * 0.056 * 1000) / 1000 };
}

// Ein Template auf 9:16 outpainten (teures Modell). Liefert die Ergebnis-Bytes (PNG) + Plan.
async function outpaint(buf, ctype) {
  ...
  return { skip: false, outBuf, top, bottom };
}
...
async function processOne(t) {
  ...
  if (!exists) {
    ...
    const om = await sharp(r.outBuf).metadata();
    estCost = Math.round(((om.width * om.height) / 1e6) * COST_PER_MP * 1000) / 1000;
  ...
  Promise.resolve().then(async () => {
    ...
    for (const t of targets) {
      ...
      try {
        ...
        cur.swapped++; cur.estCost = Math.round((cur.estCost + (r.estCost || 0)) * 1000) / 1000;
        cur.results.push({ file: t.file, name: t.name, ok: true, newFile: r.newRel });
        log("OK: " + t.file + " -> " + r.newRel);
      } catch (e) {
        cur.failed++;
...
function batchStatus() {
  ...
  return {
    ...
    current: job.current, estCost: job.estCost, error: job.error,
