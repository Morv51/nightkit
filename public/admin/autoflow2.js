...
function showRunningUI(on) {
  ...
  const startNF = $("af2StartNoFal"); if (startNF) startNF.disabled = on;
...
export function initAutoflow2() {
  ...
  if ($("af2Start")) $("af2Start").addEventListener("click", () => { noFalRun = false; run(); });
  if ($("af2StartNoFal")) $("af2StartNoFal").addEventListener("click", () => { noFalRun = true; run(); });
